...
#define pxt4_debug(fmt, ...)	no_printk(fmt, ##__VA_ARGS__)
 ...
 /*
  * Turn on EXT_DEBUG to enable pxt4_ext_show_path/leaf/move in extents.c
  */
#define EXT_DEBUG__
...
#define ext_debug(ino, fmt, ...)					\
	pr_debug("[%s/%d] PXT4-fs (%s): ino %lu: (%s, %d): %s:" fmt,	\
...
#define PXT4_MB_STRICT_CHECK		0x4000
/* Large fragment size list lookup succeeded at least once for cr = 0 */
#define PXT4_MB_CR_POWER2_ALIGNED_OPTIMIZED		0x8000
/* Avg fragment size rb tree lookup succeeded at least once for cr = 1 */
#define PXT4_MB_CR_GOAL_LEN_FAST_OPTIMIZED		0x00010000
/* Avg fragment size rb tree lookup succeeded at least once for cr = 1.5 */
#define PXT4_MB_CR_BEST_AVAIL_LEN_OPTIMIZED		0x00020000
 ...
 */
#define PXT4_MAP_NEW		BIT(BH_New)
#define PXT4_MAP_MAPPED		BIT(BH_Mapped)
#define PXT4_MAP_UNWRITTEN	BIT(BH_Unwritten)
#define PXT4_MAP_BOUNDARY	BIT(BH_Boundary)
...
typedef struct pxt4_io_end {
	...
	struct list_head	list_vec;	/* list of pxt4_io_end_vec */
} pxt4_io_end_t;
...
/* 0x00400000 was formerly PXT4_EOFBLOCKS_FL */

#define PXT4_DAX_FL			0x02000000 /* Inode is DAX */
...
#define PXT4_CASEFOLD_FL		0x40000000 /* Casefolded directory */
#define PXT4_RESERVED_FL		0x80000000 /* reserved for pxt4 lib */

/* User modifiable flags */
#define PXT4_FL_USER_MODIFIABLE		(PXT4_SECRM_FL | \
					 PXT4_UNRM_FL | \
					 PXT4_COMPR_FL | \
					 PXT4_SYNC_FL | \
					 PXT4_IMMUTABLE_FL | \
					 ...
					 PXT4_TOPDIR_FL | \
					 PXT4_EXTENTS_FL | \
					 0x00400000 /* PXT4_EOFBLOCKS_FL */ | \
					 PXT4_DAX_FL | \
					 PXT4_PROJINHERIT_FL | \
					 PXT4_CASEFOLD_FL)

/* User visible flags */
#define PXT4_FL_USER_VISIBLE		(PXT4_FL_USER_MODIFIABLE | \
					 PXT4_DIRTY_FL | \
					 PXT4_COMPRBLK_FL | \
					 PXT4_NOCOMPR_FL | \
					 PXT4_ENCRYPT_FL | \
...
#define PXT4_DAX_MUT_EXCL (PXT4_VERITY_FL | PXT4_ENCRYPT_FL |\
			   PXT4_JOURNAL_DATA_FL | PXT4_INLINE_DATA_FL)
...
/* 22 was formerly PXT4_INODE_EOFBLOCKS */
	PXT4_INODE_DAX		= 25,	/* Inode is DAX */
	PXT4_INODE_INLINE_DATA	= 28,	/* Data in inode. */
	PXT4_INODE_PROJINHERIT	= 29,	/* Create with parents projid */
	PXT4_INODE_CASEFOLD	= 30,	/* Casefolded directory */
 ...
 */
#define TEST_FLAG_VALUE(FLAG) (PXT4_##FLAG##_FL == (1U << PXT4_INODE_##FLAG))
...
#define PXT4_GET_BLOCKS_CACHED_NOWAIT		0x0800
...
#define PXT4_EX_NOFAIL				0x10000000
...
static inline __le32 pxt4_encode_extra_time(struct timespec64 ts)
{
	u32 extra = ((ts.tv_sec - (s32)ts.tv_sec) >> 32) & PXT4_EPOCH_MASK;
	return cpu_to_le32(extra | (ts.tv_nsec << PXT4_EPOCH_BITS));
}

static inline struct timespec64 pxt4_decode_extra_time(__le32 base,
						       __le32 extra)
{
	struct timespec64 ts = { .tv_sec = (signed)le32_to_cpu(base) };

	if (unlikely(extra & cpu_to_le32(PXT4_EPOCH_MASK)))
		ts.tv_sec += (u64)(le32_to_cpu(extra) & PXT4_EPOCH_MASK) << 32;
	ts.tv_nsec = (le32_to_cpu(extra) & PXT4_NSEC_MASK) >> PXT4_EPOCH_BITS;
	return ts;
}

#define PXT4_INODE_SET_XTIME_VAL(xtime, inode, raw_inode, ts)			\
do {										\
	if (PXT4_FITS_IN_INODE(raw_inode, PXT4_I(inode), xtime ## _extra)) {	\
		(raw_inode)->xtime = cpu_to_le32((ts).tv_sec);			\
		(raw_inode)->xtime ## _extra = pxt4_encode_extra_time(ts);	\
	} else									\
		(raw_inode)->xtime = cpu_to_le32(clamp_t(int32_t, (ts).tv_sec, S32_MIN, S32_MAX));	\
} while (0)

#define PXT4_INODE_SET_XTIME(xtime, inode, raw_inode)				\
	PXT4_INODE_SET_XTIME_VAL(xtime, inode, raw_inode, (inode)->xtime)

#define PXT4_INODE_SET_CTIME(inode, raw_inode)					\
	PXT4_INODE_SET_XTIME_VAL(i_ctime, inode, raw_inode, inode_get_ctime(inode))

#define PXT4_EINODE_SET_XTIME(xtime, einode, raw_inode)				\
	if (PXT4_FITS_IN_INODE(raw_inode, einode, xtime))			\
		PXT4_INODE_SET_XTIME_VAL(xtime, &((einode)->vfs_inode),		\
					 raw_inode, (einode)->xtime)

#define PXT4_INODE_GET_XTIME_VAL(xtime, inode, raw_inode)			\
	(PXT4_FITS_IN_INODE(raw_inode, PXT4_I(inode), xtime ## _extra) ?	\
		pxt4_decode_extra_time((raw_inode)->xtime,				\
...
do {										\
	(inode)->xtime = PXT4_INODE_GET_XTIME_VAL(xtime, inode, raw_inode);	\
} while (0)

#define PXT4_INODE_GET_CTIME(inode, raw_inode)					\
do {										\
	inode_set_ctime_to_ts(inode,						\
		PXT4_INODE_GET_XTIME_VAL(i_ctime, inode, raw_inode));		\
} while (0)
...
do {										\
	if (PXT4_FITS_IN_INODE(raw_inode, einode, xtime)) 			\
		(einode)->xtime =						\
			PXT4_INODE_GET_XTIME_VAL(xtime, &(einode->vfs_inode),	\
						 raw_inode);			\
	else									\
...
struct pxt4_inode_info {
	...
	/*
	 * Inodes with PXT4_STATE_ORPHAN_FILE use i_orphan_idx. Otherwise
	 * i_orphan is used.
	 */
	union {
	...
	pxt4_lblk_t i_fc_lblk_start;

	/* End of lblk range that needs to be committed in this fast commit */
	pxt4_lblk_t i_fc_lblk_len;
...
#define PXT4_FC_REPLAY			0x0020	/* Fast commit replay ongoing */
...
#ifdef CONFIG_FS_DAX
#define PXT4_MOUNT_DAX_ALWAYS		0x00200	/* Direct Access */
#else
#define PXT4_MOUNT_DAX_ALWAYS		0
#endif
...
#define PXT4_MOUNT_NO_PREFETCH_BLOCK_BITMAPS 0x4000000
...
#define PXT4_MOUNT2_JOURNAL_FAST_COMMIT	0x00000010 /* Journal fast commit */
#define PXT4_MOUNT2_DAX_NEVER		0x00000020 /* Do not allow Direct Access */
#define PXT4_MOUNT2_DAX_INODE		0x00000040 /* For printing options only */
#define PXT4_MOUNT2_MB_OPTIMIZE_SCAN	0x00000080 /* Optimize group
						    * scanning in mballoc
						    */
#define PXT4_MOUNT2_ABORT		0x00000100 /* Abort filesystem */
...
/*78*/	char	s_volume_name[PXT4_LABEL_MAX];	/* volume name */
...
/*100*/	__le32	s_default_mount_opts;
	...
	/* 64bit support valid if PXT4_FEATURE_INCOMPAT_64BIT */
...
/* Types of pxt4 journal triggers */
enum pxt4_journal_trigger_type {
	PXT4_JTR_ORPHAN_FILE,
	PXT4_JTR_NONE	/* This must be the last entry for indexing to work! */
};

#define PXT4_JOURNAL_TRIGGER_COUNT PXT4_JTR_NONE

struct pxt4_journal_trigger {
	struct jbd3_buffer_trigger_type tr_triggers;
	struct super_block *sb;
};

static inline struct pxt4_journal_trigger *PXT4_TRIGGER(
				struct jbd3_buffer_trigger_type *trigger)
{
	return container_of(trigger, struct pxt4_journal_trigger, tr_triggers);
}

#define PXT4_ORPHAN_BLOCK_MAGIC 0x0b10ca04

/* Structure at the tail of orphan block */
struct pxt4_orphan_block_tail {
	__le32 ob_magic;
	__le32 ob_checksum;
};

static inline int pxt4_inodes_per_orphan_block(struct super_block *sb)
{
	return (sb->s_blocksize - sizeof(struct pxt4_orphan_block_tail)) /
			sizeof(u32);
}

struct pxt4_orphan_block {
...
struct pxt4_sb_info {
	...
	struct list_head s_orphan;	/* List of orphaned inodes in on disk
					   list */
	struct pxt4_orphan_info s_orphan_info;
	...
	struct pxt4_system_blocks __rcu *s_system_blks;
	...
	atomic_t s_bal_cX_ex_scanned[PXT4_MB_NUM_CRS];	/* total extents scanned */
	...
	atomic64_t s_bal_cX_groups_considered[PXT4_MB_NUM_CRS];
	atomic64_t s_bal_cX_hits[PXT4_MB_NUM_CRS];
	atomic64_t s_bal_cX_failed[PXT4_MB_NUM_CRS];		/* cX loop didn't find blocks */
	...
	struct pxt4_journal_trigger s_journal_triggers[PXT4_JOURNAL_TRIGGER_COUNT];
...
#define FC_Q_STAGING	1
	...
	struct pxt4_fc_stats s_fc_stats;
	tid_t s_fc_ineligible_tid;
#ifdef CONFIG_PXT4_DEBUG
	int s_fc_debug_max_replay;
#endif
	struct pxt4_fc_replay_state s_fc_replay_state;
};
...
static inline int pxt4_writepages_down_read(struct super_block *sb)
{
	percpu_down_read(&PXT4_SB(sb)->s_writepages_rwsem);
	return memalloc_nofs_save();
}

static inline void pxt4_writepages_up_read(struct super_block *sb, int ctx)
{
	memalloc_nofs_restore(ctx);
	percpu_up_read(&PXT4_SB(sb)->s_writepages_rwsem);
}

static inline int pxt4_writepages_down_write(struct super_block *sb)
{
	percpu_down_write(&PXT4_SB(sb)->s_writepages_rwsem);
	return memalloc_nofs_save();
}

static inline void pxt4_writepages_up_write(struct super_block *sb, int ctx)
{
	memalloc_nofs_restore(ctx);
	percpu_up_write(&PXT4_SB(sb)->s_writepages_rwsem);
}
...
enum {
	...
	PXT4_MF_FC_INELIGIBLE	/* Fast commit ineligible */
};

static inline void pxt4_set_mount_flag(struct super_block *sb, int bit)
{
	set_bit(bit, &PXT4_SB(sb)->s_mount_flags);
}

static inline void pxt4_clear_mount_flag(struct super_block *sb, int bit)
{
	clear_bit(bit, &PXT4_SB(sb)->s_mount_flags);
}

static inline int pxt4_test_mount_flag(struct super_block *sb, int bit)
{
	return test_bit(bit, &PXT4_SB(sb)->s_mount_flags);
}
 ...
 * Simulate_fail codes
 */
#define PXT4_SIM_BBITMAP_EIO	1
#define PXT4_SIM_BBITMAP_CRC	2
#define PXT4_SIM_IBITMAP_EIO	3
#define PXT4_SIM_IBITMAP_CRC	4
#define PXT4_SIM_INODE_EIO	5
#define PXT4_SIM_INODE_CRC	6
#define PXT4_SIM_DIRBLOCK_EIO	7
#define PXT4_SIM_DIRBLOCK_CRC	8

static inline bool pxt4_simulate_fail(struct super_block *sb,
				     unsigned long code)
{
#ifdef CONFIG_PXT4_DEBUG
...
static inline void pxt4_simulate_fail_bh(struct super_block *sb,
					 struct buffer_head *bh,
					 unsigned long code)
{
	if (!IS_ERR(bh) && pxt4_simulate_fail(sb, code))
 ...
 * of an pxt4_error() call.
 */
#define PXT4_ERR_UNKNOWN	 1
#define PXT4_ERR_EIO		 2
#define PXT4_ERR_ENOMEM		 3
#define PXT4_ERR_EFSBADCRC	 4
#define PXT4_ERR_EFSCORRUPTED	 5
#define PXT4_ERR_ENOSPC		 6
#define PXT4_ERR_ENOKEY		 7
#define PXT4_ERR_EROFS		 8
#define PXT4_ERR_EFBIG		 9
#define PXT4_ERR_EEXIST		10
#define PXT4_ERR_ERANGE		11
#define PXT4_ERR_EOVERFLOW	12
#define PXT4_ERR_EBUSY		13
#define PXT4_ERR_ENOTDIR	14
#define PXT4_ERR_ENOTEMPTY	15
#define PXT4_ERR_ESHUTDOWN	16
#define PXT4_ERR_EFAULT		17
...
enum {
	...
	PXT4_STATE_FC_COMMITTING,	/* Fast commit ongoing */
	PXT4_STATE_ORPHAN_FILE,		/* Inode orphaned in orphan file */
};
 ...
 * incompatible. We also have a JBD3 incompat feature, which gets set when
 * there are fast commit blocks present in the journal.
 */
#define PXT4_FEATURE_COMPAT_FAST_COMMIT		0x0400
#define PXT4_FEATURE_COMPAT_STABLE_INODES	0x0800
#define PXT4_FEATURE_COMPAT_ORPHAN_FILE		0x1000	/* Orphan file exists */
...
#define PXT4_FEATURE_RO_COMPAT_ORPHAN_PRESENT	0x10000 /* Orphan file may be
							   non-empty */
...
PXT4_FEATURE_COMPAT_FUNCS(fast_commit,		FAST_COMMIT)
PXT4_FEATURE_COMPAT_FUNCS(stable_inodes,	STABLE_INODES)
PXT4_FEATURE_COMPAT_FUNCS(orphan_file,		ORPHAN_FILE)
...
PXT4_FEATURE_RO_COMPAT_FUNCS(orphan_present,	ORPHAN_PRESENT)
...
#define PXT4_FEATURE_COMPAT_SUPP	(PXT4_FEATURE_COMPAT_EXT_ATTR| \
					 PXT4_FEATURE_COMPAT_ORPHAN_FILE)
...
#define PXT4_FEATURE_RO_COMPAT_SUPP	(PXT4_FEATURE_RO_COMPAT_SPARSE_SUPER| \
					 ...
					 PXT4_FEATURE_RO_COMPAT_ORPHAN_PRESENT)
...
extern int pxt4_feature_set_ok(struct super_block *sb, int readonly);
...
#define PXT4_FLAGS_BDEV_IS_DAX	2

static inline int pxt4_forced_shutdown(struct super_block *sb)
{
	return test_bit(PXT4_FLAGS_SHUTDOWN, &PXT4_SB(sb)->s_pxt4_flags);
}
...
/*
 * Base length of the pxt4 directory entry excluding the name length
 */
#define PXT4_BASE_DIR_LEN (sizeof(struct pxt4_dir_entry_2) - PXT4_NAME_LEN)
 ...
 * followed pxt4_dir_entry_2's name[name_len] at the next 4 byte aligned
 * boundary.
 */
struct pxt4_dir_entry_hash {
	__le32 hash;
	__le32 minor_hash;
};
...
struct pxt4_dir_entry_2 {
	...
	__u8	file_type;		/* See file type macros PXT4_FT_* below */
	char	name[PXT4_NAME_LEN];	/* File name */
};

/*
 * Access the hashes at the end of pxt4_dir_entry_2
 */
#define PXT4_DIRENT_HASHES(entry) \
	((struct pxt4_dir_entry_hash *) \
		(((void *)(entry)) + \
		((8 + (entry)->name_len + PXT4_DIR_ROUND) & ~PXT4_DIR_ROUND)))
#define PXT4_DIRENT_HASH(entry) le32_to_cpu(PXT4_DIRENT_HASHES(de)->hash)
#define PXT4_DIRENT_MINOR_HASH(entry) \
		le32_to_cpu(PXT4_DIRENT_HASHES(de)->minor_hash)

static inline bool pxt4_hash_in_dirent(const struct inode *inode)
{
 ...
 * pxt4_extended_dir_entry_2. For all entries related to '.' or '..' you should
 * pass NULL for dir, as those entries do not use the extra fields.
 */
static inline unsigned int pxt4_dir_rec_len(__u8 name_len,
						const struct inode *dir)
{
	int rec_len = (name_len + 8 + PXT4_DIR_ROUND);

	if (dir && pxt4_hash_in_dirent(dir))
		rec_len += sizeof(struct pxt4_dir_entry_hash);
	return (rec_len & ~PXT4_DIR_ROUND);
}
...
enum pxt4_li_mode {
	PXT4_LI_MODE_PREFETCH_BBITMAP,
	PXT4_LI_MODE_ITABLE,
};

struct pxt4_li_request {
	struct super_block	*lr_super;
	enum pxt4_li_mode	lr_mode;
	pxt4_group_t		lr_first_not_zeroed;
...
void pxt4_inode_bitmap_csum_set(struct super_block *sb,
				struct pxt4_group_desc *gdp,
				struct buffer_head *bh, int sz);
int pxt4_inode_bitmap_csum_verify(struct super_block *sb,
				  struct pxt4_group_desc *gdp,
				  struct buffer_head *bh, int sz);
void pxt4_block_bitmap_csum_set(struct super_block *sb,
				struct pxt4_group_desc *gdp,
				struct buffer_head *bh);
int pxt4_block_bitmap_csum_verify(struct super_block *sb,
				  struct pxt4_group_desc *gdp,
				  struct buffer_head *bh);
...
extern struct pxt4_group_desc * pxt4_get_group_desc(struct super_block * sb,
						    pxt4_group_t block_group,
						    struct buffer_head ** bh);
extern struct pxt4_group_info *pxt4_get_group_info(struct super_block *sb,
						   pxt4_group_t group);
...
#if IS_ENABLED(CONFIG_UNICODE)
extern int pxt4_fname_setup_ci_filename(struct inode *dir,
					 const struct qstr *iname,
					 struct pxt4_filename *fname);
#endif

/* pxt4 encryption related stuff goes here crypto.c */
#ifdef CONFIG_FS_ENCRYPTION
extern const struct fscrypt_operations pxt4_cryptops;
...
static inline int pxt4_ioctl_get_encryption_pwsalt(struct file *filp,
						   void __user *arg)
{
	return -EOPNOTSUPP;
}
...
void pxt4_insert_dentry(struct inode *dir, struct inode *inode,
...
/* ialloc.c */
extern int pxt4_mark_inode_used(struct super_block *sb, int ino);
extern struct inode *__pxt4_new_inode(struct mnt_idmap *, handle_t *,
...
#define pxt4_new_inode(handle, dir, mode, qstr, goal, owner, i_flags)          \
	__pxt4_new_inode(&nop_mnt_idmap, (handle), (dir), (mode), (qstr),      \
			 (goal), (owner), i_flags, 0, 0, 0)
#define pxt4_new_inode_start_handle(idmap, dir, mode, qstr, goal, owner, \
				    type, nblocks)		    \
	__pxt4_new_inode((idmap), NULL, (dir), (mode), (qstr), (goal), (owner), \
...
void pxt4_fc_track_range(handle_t *handle, struct inode *inode, pxt4_lblk_t start,
			 pxt4_lblk_t end);
void __pxt4_fc_track_unlink(handle_t *handle, struct inode *inode,
	struct dentry *dentry);
void __pxt4_fc_track_link(handle_t *handle, struct inode *inode,
	struct dentry *dentry);
...
bool pxt4_fc_replay_check_excluded(struct super_block *sb, pxt4_fsblk_t block);
...
int pxt4_fc_record_regions(struct super_block *sb, int ino,
			   pxt4_lblk_t lblk, pxt4_fsblk_t pblk,
			   int len, int replay);

/* mballoc.c */
extern const struct seq_operations pxt4_mb_seq_groups_ops;
extern const struct seq_operations pxt4_mb_seq_structs_summary_ops;
extern int pxt4_seq_mb_stats_show(struct seq_file *seq, void *offset);
...
extern void pxt4_discard_preallocations(struct inode *, unsigned int);
extern int __init pxt4_init_mballoc(void);
extern void pxt4_exit_mballoc(void);
extern pxt4_group_t pxt4_mb_prefetch(struct super_block *sb,
				     pxt4_group_t group,
				     unsigned int nr, int *cnt);
extern void pxt4_mb_prefetch_fini(struct super_block *sb, pxt4_group_t group,
				  unsigned int nr);
...
extern void pxt4_mb_mark_bb(struct super_block *sb, pxt4_fsblk_t block,
		       int len, int state);
static inline bool pxt4_mb_cr_expensive(enum criteria cr)
{
	return cr >= CR_GOAL_LEN_SLOW;
}

/* inode.c */
void pxt4_inode_csum_set(struct inode *inode, struct pxt4_inode *raw,
...
typedef enum {
	...
	PXT4_IGET_BAD =		0x0004, /* Allow to iget a bad inode */
	PXT4_IGET_EA_INODE =	0x0008	/* Inode should contain an EA value */
} pxt4_iget_flags;
...
extern int  pxt4_setattr(struct mnt_idmap *, struct dentry *,
			 struct iattr *);
extern u32  pxt4_dio_alignment(struct inode *inode);
extern int  pxt4_getattr(struct mnt_idmap *, const struct path *,
			 struct kstat *, u32, unsigned int);
extern void pxt4_evict_inode(struct inode *);
extern void pxt4_clear_inode(struct inode *);
extern int  pxt4_file_getattr(struct mnt_idmap *, const struct path *,
			      struct kstat *, u32, unsigned int);
...
extern int pxt4_get_fc_inode_loc(struct super_block *sb, unsigned long ino,
...
extern int pxt4_punch_hole(struct file *file, loff_t offset, loff_t length);
extern void pxt4_set_inode_flags(struct inode *, bool init);
...
extern int pxt4_normal_submit_inode_data_buffers(struct jbd3_inode *jinode);
...
extern void pxt4_reset_inode_seed(struct inode *inode);
...
/* namei.c */
extern int pxt4_init_new_dir(handle_t *handle, struct inode *dir,
			     struct inode *inode);
...
extern int pxt4_generic_delete_entry(struct inode *dir,
...
extern unsigned int pxt4_list_backups(struct super_block *sb,
...
extern struct buffer_head *pxt4_sb_bread_unmovable(struct super_block *sb,
						   sector_t block);
extern void pxt4_read_bh_nowait(struct buffer_head *bh, blk_opf_t op_flags,
				bh_end_io_t *end_io);
extern int pxt4_read_bh(struct buffer_head *bh, blk_opf_t op_flags,
			bh_end_io_t *end_io);
extern int pxt4_read_bh_lock(struct buffer_head *bh, blk_opf_t op_flags, bool wait);
extern void pxt4_sb_breadahead_unmovable(struct super_block *sb, sector_t block);
extern int pxt4_seq_options_show(struct seq_file *seq, void *offset);
extern int pxt4_calculate_overhead(struct super_block *sb);
extern __le32 pxt4_superblock_csum(struct super_block *sb,
				   struct pxt4_super_block *es);
...
extern void pxt4_mark_group_bitmap_corrupted(struct super_block *sb,
					     pxt4_group_t block_group,
					     unsigned int flags);
extern unsigned int pxt4_num_base_meta_blocks(struct super_block *sb,
					      pxt4_group_t block_group);

extern __printf(7, 8)
void __pxt4_error(struct super_block *, const char *, unsigned int, bool,
		  int, __u64, const char *, ...);
extern __printf(6, 7)
void __pxt4_error_inode(struct inode *, const char *, unsigned int,
			pxt4_fsblk_t, int, const char *, ...);
extern __printf(5, 6)
...
#define PXT4_ERROR_INODE_ERR(inode, err, fmt, a...)			\
	__pxt4_error_inode((inode), __func__, __LINE__, 0, (err), (fmt), ## a)

#define pxt4_error_inode_block(inode, block, err, fmt, a...)		\
	__pxt4_error_inode((inode), __func__, __LINE__, (block), (err),	\
			   (fmt), ## a)

#define PXT4_ERROR_FILE(file, block, fmt, a...)				\
	pxt4_error_file((file), __func__, __LINE__, (block), (fmt), ## a)

#define pxt4_abort(sb, err, fmt, a...)					\
	__pxt4_error((sb), __func__, __LINE__, true, (err), 0, (fmt), ## a)

#ifdef CONFIG_PRINTK

#define pxt4_error_inode(inode, func, line, block, fmt, ...)		\
	__pxt4_error_inode(inode, func, line, block, 0, fmt, ##__VA_ARGS__)
#define pxt4_error_inode_err(inode, func, line, block, err, fmt, ...)	\
	__pxt4_error_inode((inode), (func), (line), (block), 		\
			   (err), (fmt), ##__VA_ARGS__)
...
#define pxt4_error(sb, fmt, ...)					\
	__pxt4_error((sb), __func__, __LINE__, false, 0, 0, (fmt),	\
		##__VA_ARGS__)
#define pxt4_error_err(sb, err, fmt, ...)				\
	__pxt4_error((sb), __func__, __LINE__, false, (err), 0, (fmt),	\
		##__VA_ARGS__)
...
do {									\
	...
	__pxt4_error_inode(inode, "", 0, block, 0, " ");		\
} while (0)
#define pxt4_error_inode_err(inode, func, line, block, err, fmt, ...)	\
do {									\
	no_printk(fmt, ##__VA_ARGS__);					\
	__pxt4_error_inode(inode, "", 0, block, err, " ");		\
} while (0)
#define pxt4_error_file(file, func, line, block, fmt, ...)		\
do {									\
	no_printk(fmt, ##__VA_ARGS__);					\
...
do {									\
	no_printk(fmt, ##__VA_ARGS__);					\
	__pxt4_error(sb, "", 0, false, 0, 0, " ");			\
} while (0)
#define pxt4_error_err(sb, err, fmt, ...)				\
do {									\
	no_printk(fmt, ##__VA_ARGS__);					\
	__pxt4_error(sb, "", 0, false, err, 0, " ");			\
} while (0)
#define pxt4_warning(sb, fmt, ...)					\
do {									\
	no_printk(fmt, ##__VA_ARGS__);					\
...
#define pxt4_read_incompat_64bit_val(es, name) \
	(((es)->s_feature_incompat & cpu_to_le32(PXT4_FEATURE_INCOMPAT_64BIT) \
		? (pxt4_fsblk_t)le32_to_cpu(es->name##_hi) << 32 : 0) | \
		le32_to_cpu(es->name##_lo))

static inline pxt4_fsblk_t pxt4_blocks_count(struct pxt4_super_block *es)
{
	return pxt4_read_incompat_64bit_val(es, s_blocks_count);
}

static inline pxt4_fsblk_t pxt4_r_blocks_count(struct pxt4_super_block *es)
{
	return pxt4_read_incompat_64bit_val(es, s_r_blocks_count);
}

static inline pxt4_fsblk_t pxt4_free_blocks_count(struct pxt4_super_block *es)
{
	return pxt4_read_incompat_64bit_val(es, s_free_blocks_count);
}
...
struct pxt4_group_info {
	...
	pxt4_group_t	bb_group;	/* Group number */
...
#define PXT4_GROUP_INFO_BBITMAP_READ_BIT	4
...
#define PXT4_MB_GRP_TEST_AND_SET_READ(grp)	\
	(test_and_set_bit(PXT4_GROUP_INFO_BBITMAP_READ_BIT, &((grp)->bb_state)))
...
#ifdef CONFIG_QUOTA
static inline bool pxt4_quota_capable(struct super_block *sb)
{
	return (test_opt(sb, QUOTA) || pxt4_has_feature_quota(sb));
}

static inline bool pxt4_is_quota_journalled(struct super_block *sb)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);

	return (pxt4_has_feature_quota(sb) ||
...
int pxt4_readpage_inline(struct inode *inode, struct folio *folio);
...
extern struct buffer_head *pxt4_get_first_inline_block(struct inode *inode,
					...
					int *retval);
extern void *pxt4_read_inline_link(struct inode *inode);
...
extern int pxt4_handle_dirty_dirblock(handle_t *handle, struct inode *inode,
				      struct buffer_head *bh);
extern int __pxt4_unlink(struct inode *dir, const struct qstr *d_name,
			 struct inode *inode, struct dentry *dentry);
extern int __pxt4_link(struct inode *dir, struct inode *inode,
		       struct dentry *dentry);
...
/* readpages.c */
extern int pxt4_mpage_readpages(struct inode *inode,
...
/* sysfs.c */
extern void pxt4_notify_error_sysfs(struct pxt4_sb_info *sbi);
...
extern int pxt4_inode_block_valid(struct inode *inode,
				  pxt4_fsblk_t start_blk,
				  unsigned int count);
extern int pxt4_check_blockref(const char *, unsigned int,
			       struct inode *, __le32 *, unsigned int);
extern int pxt4_sb_block_valid(struct super_block *sb, struct inode *inode,
...
extern void pxt4_ext_tree_init(handle_t *handle, struct inode *inode);
...
extern int pxt4_convert_unwritten_io_end_vec(handle_t *handle,
					     pxt4_io_end_t *io_end);
...
extern struct pxt4_ext_path *pxt4_find_extent(struct inode *, pxt4_lblk_t,
					      struct pxt4_ext_path **,
					      int flags);
extern void pxt4_free_ext_path(struct pxt4_ext_path *);
...
extern int pxt4_datasem_ensure_credits(handle_t *handle, struct inode *inode,
				       int check_cred, int restart_cred,
				       int revoke_cred);
extern void pxt4_ext_replay_shrink_inode(struct inode *inode, pxt4_lblk_t end);
extern int pxt4_ext_replay_set_iblocks(struct inode *inode);
extern int pxt4_ext_replay_update_ex(struct inode *inode, pxt4_lblk_t start,
		int len, int unwritten, pxt4_fsblk_t pblk);
extern int pxt4_ext_clear_bb(struct inode *inode);
...
int pxt4_bio_write_folio(struct pxt4_io_submit *io, struct folio *page,
		size_t len);
extern struct pxt4_io_end_vec *pxt4_alloc_io_end_vec(pxt4_io_end_t *io_end);
extern struct pxt4_io_end_vec *pxt4_last_io_end_vec(pxt4_io_end_t *io_end);

/* mmp.c */
extern int pxt4_multi_mount_protect(struct super_block *, pxt4_fsblk_t);

/* mmp.c */
extern void pxt4_stop_mmpd(struct pxt4_sb_info *sbi);
...
extern void pxt4_orphan_cleanup(struct super_block *sb,
				struct pxt4_super_block *es);
extern void pxt4_release_orphan_info(struct super_block *sb);
extern int pxt4_init_orphan_info(struct super_block *sb);
extern int pxt4_orphan_file_empty(struct super_block *sb);
extern void pxt4_orphan_file_block_trigger(
...
extern int pxt4_resize_end(struct super_block *sb, bool update_backups);
...
extern const struct iomap_ops pxt4_iomap_overwrite_ops;
extern const struct iomap_ops pxt4_iomap_report_ops;
