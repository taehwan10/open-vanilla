 ...
 *
 * The pxt4_mpage_readpages() function here is intended to
...
static void pxt4_set_bio_post_read_ctx(struct bio *bio,
...
int pxt4_mpage_readpages(struct inode *inode,
		struct readahead_control *rac, struct folio *folio)
{
	...
	for (; nr_pages; nr_pages--) {
		...
		if (first_hole != blocks_per_page) {
			folio_zero_segment(folio, first_hole << blkbits,
					  folio_size(folio));
			if (first_hole == 0) {
				if (pxt4_need_verity(inode, folio->index) &&
		...
		if (bio == NULL) {
			...
			fscrypt_set_bio_crypt_ctx(bio, inode, next_block,
						  GFP_KERNEL);
			pxt4_set_bio_post_read_ctx(bio, inode, folio->index);
	...
	confused:
		...
		if (!folio_test_uptodate(folio))
			block_read_full_folio(folio, pxt4_get_block);
		else
			folio_unlock(folio);
next_page:
