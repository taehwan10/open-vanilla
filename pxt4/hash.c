 ...
 */
static int __pxt4fs_dirhash(const struct inode *dir, const char *name, int len,
			    struct dx_hash_info *hinfo)
{
	...
	case DX_HASH_SIPHASH:
	{
		...
		if (fscrypt_has_encryption_key(dir)) {
			...
			pxt4_warning_inode(dir, "Siphash requires key");
			return -1;
		}
		   ...
		   struct dx_hash_info *hinfo)
{
	...
	if (len && IS_CASEFOLDED(dir) &&
	   (!IS_ENCRYPTED(dir) || fscrypt_has_encryption_key(dir))) {
		...
		r = __pxt4fs_dirhash(dir, buff, dlen, hinfo);
...
opaque_seq:
#endif
	return __pxt4fs_dirhash(dir, name, len, hinfo);
}
