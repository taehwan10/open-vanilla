...
#define mb_debug(sb, fmt, ...)						\
	pr_debug("[%s/%d] PXT4-fs (%s): (%s, %d): %s: " fmt,		\
 ...
 * We can tune the same via /proc/fs/pxt4/<partition>/stream_req
 */
...
struct pxt4_allocation_context {
	 ...
	 */
	pxt4_grpblk_t	ac_orig_goal_len;
	...
	__u16 ac_found;
	__u16 ac_cX_found[PXT4_MB_NUM_CRS];
	__u16 ac_tail;
	__u16 ac_buddy;
	__u8 ac_status;
	__u8 ac_criteria;
...
static inline loff_t extent_logical_end(struct pxt4_sb_info *sbi,
					struct pxt4_free_extent *fex)
{
	/* Use loff_t to avoid end exceeding pxt4_lblk_t max. */
	return (loff_t)fex->fe_logical + PXT4_C2B(sbi, fex->fe_len);
}

static inline loff_t pa_logical_end(struct pxt4_sb_info *sbi,
				    struct pxt4_prealloc_space *pa)
{
	/* Use loff_t to avoid end exceeding pxt4_lblk_t max. */
	return (loff_t)pa->pa_lstart + PXT4_C2B(sbi, pa->pa_len);
}
