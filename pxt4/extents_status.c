...
static int __revise_pending(struct inode *inode, pxt4_lblk_t lblk,
			    pxt4_lblk_t len,
			    struct pending_reservation **prealloc);

int __init pxt4_init_es(void)
{
	pxt4_es_cachep = KMEM_CACHE(extent_status, SLAB_RECLAIM_ACCOUNT);
...
{
	if (!nofail)
		return kmem_cache_alloc(pxt4_pending_cachep, GFP_ATOMIC);

	return kmem_cache_zalloc(pxt4_pending_cachep, GFP_KERNEL | __GFP_NOFAIL);
}
 ...
 */
static inline bool pxt4_es_must_keep(struct extent_status *es)
{
...
{
	if (!nofail)
		return kmem_cache_alloc(pxt4_es_cachep, GFP_ATOMIC);

	return kmem_cache_zalloc(pxt4_es_cachep, GFP_KERNEL | __GFP_NOFAIL);
}

static void pxt4_es_init_extent(struct inode *inode, struct extent_status *es,
		pxt4_lblk_t lblk, pxt4_lblk_t len, pxt4_fsblk_t pblk)
{
	es->es_lblk = lblk;
	es->es_len = len;
	es->es_pblk = pblk;
			      ...
			      struct extent_status *prealloc)
{
	...
	if (!es)
		return -ENOMEM;
	pxt4_es_init_extent(inode, es, newes->es_lblk, newes->es_len,
			    newes->es_pblk);
 ...
 * status tree.
 */
void pxt4_es_insert_extent(struct inode *inode, pxt4_lblk_t lblk,
...
retry:
	...
	if (err2 == -ENOMEM && !pxt4_es_must_keep(&newes))
		err2 = 0;
	if (err2 != 0)
		goto error;
	/* Free preallocated extent if it didn't get used. */
 ...
 */
void pxt4_es_remove_extent(struct inode *inode, pxt4_lblk_t lblk,
			   pxt4_lblk_t len)
{
	pxt4_lblk_t end;
	int err = 0;
...
int pxt4_es_register_shrinker(struct pxt4_sb_info *sbi)
{
	...
	err = register_shrinker(&sbi->s_es_shrinker, "pxt4-es:%s",
				sbi->s_sb->s_id);
	if (err)
		goto err4;
...
{
	pxt4_pending_cachep = KMEM_CACHE(pending_reservation, SLAB_RECLAIM_ACCOUNT);
 ...
 */
void pxt4_es_insert_delayed_block(struct inode *inode, pxt4_lblk_t lblk,
				  bool allocated)
{
	struct extent_status newes;
	int err1 = 0, err2 = 0, err3 = 0;
