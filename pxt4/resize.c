...
int pxt4_resize_begin(struct super_block *sb)
{
	 ...
	 */
	if (PXT4_SB(sb)->s_es->s_reserved_gdt_blocks &&
	    !pxt4_has_feature_resize_inode(sb)) {
		pxt4_error(sb, "resize_inode disabled but reserved GDT blocks non-zero");
		return -EFSCORRUPTED;
	}
	...
	if (pxt4_has_feature_sparse_super2(sb)) {
		pxt4_msg(sb, KERN_ERR, "Online resizing not supported with sparse_super2");
		return -EOPNOTSUPP;
	}
...
int pxt4_resize_end(struct super_block *sb, bool update_backups)
{
	clear_bit_unlock(PXT4_FLAGS_RESIZING, &PXT4_SB(sb)->s_pxt4_flags);
	smp_mb__after_atomic();
	if (update_backups)
		return pxt4_update_overhead(sb, true);
	return 0;
}
...
static int pxt4_resize_ensure_credits_batch(handle_t *handle, int credits)
{
	return pxt4_journal_ensure_credits_fn(handle, credits,
		PXT4_MAX_TRANS_DATA, 0, 0);
}
			...
			pxt4_fsblk_t first_cluster, pxt4_fsblk_t last_cluster)
{
	...
	for (count2 = count; count > 0;
		...
		err = pxt4_resize_ensure_credits_batch(handle, 1);
		if (err < 0)
			return err;
		...
		BUFFER_TRACE(bh, "get_write_access");
		err = pxt4_journal_get_write_access(handle, sb, bh,
						    PXT4_JTR_NONE);
		if (err) {
			brelse(bh);
			return err;
				...
				struct pxt4_new_flex_group_data *flex_gd)
{
	...
	for (i = 0; i < flex_gd->count; i++, group++) {
		...
		for (j = 0; j < gdblocks; j++, block++) {
			...
			err = pxt4_journal_get_write_access(handle, sb, gdb,
							    PXT4_JTR_NONE);
			if (err) {
				brelse(gdb);
				goto out;
 ...
 */
unsigned int pxt4_list_backups(struct super_block *sb, unsigned int *three,
		       ...
		       pxt4_group_t group)
{
	...
	err = pxt4_journal_get_write_access(handle, sb, PXT4_SB(sb)->s_sbh,
					    PXT4_JTR_NONE);
	if (unlikely(err))
		goto errout;

	BUFFER_TRACE(gdb_bh, "get_write_access");
	err = pxt4_journal_get_write_access(handle, sb, gdb_bh, PXT4_JTR_NONE);
	if (unlikely(err))
		goto errout;

	BUFFER_TRACE(dind, "get_write_access");
	err = pxt4_journal_get_write_access(handle, sb, dind, PXT4_JTR_NONE);
	if (unlikely(err)) {
	...
	err = pxt4_handle_dirty_metadata(handle, NULL, PXT4_SB(sb)->s_sbh);
	if (err)
		pxt4_std_error(sb, err);
	return err;
errout:
			      ...
			      pxt4_group_t group)
{
	...
	for (i = 0; i < reserved_gdb; i++) {
		...
		if ((err = pxt4_journal_get_write_access(handle, sb, primary[i],
							 PXT4_JTR_NONE)))
			goto exit_bh;
	}
...
static inline void pxt4_set_block_group_nr(struct super_block *sb, char *data,
					   pxt4_group_t group)
{
	struct pxt4_super_block *es = (struct pxt4_super_block *) data;
			   ...
			   int size, int meta_bg)
{
	...
	while (group < sbi->s_groups_count) {
		...
		int has_super = pxt4_bg_has_super(sb, group);
		pxt4_fsblk_t first_block = pxt4_group_first_block_no(sb, group);
		...
		BUFFER_TRACE(bh, "get_write_access");
		if ((err = pxt4_journal_get_write_access(handle, sb, bh,
							 PXT4_JTR_NONE))) {
			brelse(bh);
			break;
		}
		...
		if (has_super && (backup_block == first_block))
			pxt4_set_block_group_nr(sb, bh->b_data, group);
		set_buffer_uptodate(bh);
		unlock_buffer(bh);
		err = pxt4_handle_dirty_metadata(handle, NULL, bh);
		if (unlikely(err))
	...
	for (i = 0; i < count; i++, group++) {
		...
		if (gdb_off) {
			gdb_bh = sbi_array_rcu_deref(sbi, s_group_desc,
						     gdb_num);
			BUFFER_TRACE(gdb_bh, "get_write_access");
			err = pxt4_journal_get_write_access(handle, sb, gdb_bh,
							    PXT4_JTR_NONE);
				     ...
				     struct pxt4_new_group_data *group_data)
{
	...
	if (!bh)
		return -EIO;
	pxt4_inode_bitmap_csum_set(sb, gdp, bh,
	...
	if (!bh)
		return -EIO;
	pxt4_block_bitmap_csum_set(sb, gdp, bh);
	brelse(bh);
				...
				struct pxt4_new_flex_group_data *flex_gd)
{
	...
	for (i = 0; i < flex_gd->count; i++, group_data++, bg_flags++) {
		...
		err = pxt4_set_bitmap_checksums(sb, gdp, group_data);
		if (err) {
			pxt4_std_error(sb, err);
			break;
		}
...
static void pxt4_add_overhead(struct super_block *sb,
                              const pxt4_fsblk_t overhead)
{
       struct pxt4_sb_info *sbi = PXT4_SB(sb);
       struct pxt4_super_block *es = sbi->s_es;
			     ...
			     struct pxt4_new_flex_group_data *flex_gd)
{
	 ...
	 */
	if (pxt4_has_feature_bigalloc(sb) && (sbi->s_overhead != 0))
		pxt4_add_overhead(sb,
			PXT4_NUM_B2C(sbi, blocks_count - free_blocks));
	else
...
exit_journal:
	...
	if (!err) {
		...
		sector_t padding_blocks = meta_bg ? 0 : sbi->s_sbh->b_blocknr -
					 pxt4_group_first_block_no(sb, 0);
		sector_t old_gdb = 0;

		update_backups(sb, pxt4_group_first_block_no(sb, 0),
		      ...
		      pxt4_fsblk_t n_blocks_count)
{
	...
	bh = pxt4_sb_bread(sb, o_blocks_count + add - 1, 0);
	if (IS_ERR(bh)) {
	...
	return pxt4_group_extend_no_check(sb, o_blocks_count, add);
} /* pxt4_group_extend */
...
int pxt4_resize_fs(struct super_block *sb, pxt4_fsblk_t n_blocks_count)
{
	...
	/* See if the device is actually as big as what was requested */
	bh = pxt4_sb_bread(sb, n_blocks_count - 1, 0);
	if (IS_ERR(bh)) {
		pxt4_warning(sb, "can't read last block, resize aborted");
		return -ENOSPC;
	}
	...
	if (pxt4_has_feature_bigalloc(sb))
		n_blocks_count &= ~((1 << PXT4_CLUSTER_BITS(sb)) - 1);
...
retry:
	...
	if (pxt4_blocks_count(es) == n_blocks_count && n_blocks_count_retry == 0)
		goto out;
