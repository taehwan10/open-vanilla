...
static bool is_fake_dir_entry(struct pxt4_dir_entry_2 *de)
{
	...
	if (de->file_type == PXT4_FT_DIR_CSUM)
		return true;
	return false;
}
			   ...
			   unsigned int offset)
{
	...
	bool has_csum = pxt4_has_metadata_csum(dir->i_sb);

	if (unlikely(rlen < pxt4_dir_rec_len(1, fake ? NULL : dir)))
	...
	else if (unlikely(next_offset > size - pxt4_dir_rec_len(1,
						  has_csum ? NULL : dir) &&
			  next_offset != size))
...
static int pxt4_readdir(struct file *file, struct dir_context *ctx)
{
	...
	if (IS_ENCRYPTED(inode)) {
		err = fscrypt_fname_alloc_buffer(PXT4_NAME_LEN, &fstr);
		if (err < 0)
			return err;
	}
