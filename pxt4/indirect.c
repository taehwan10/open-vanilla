				 ...
				 Indirect chain[4], int *err)
{
	...
	*err = 0;
	...
	while (--depth) {
		...
		if (key > pxt4_blocks_count(PXT4_SB(sb)->s_es)) {
			     ...
			     Indirect *branch)
{
	...
	for (i = 0; i <= indirect_blks; i++) {
		...
		err = pxt4_journal_get_create_access(handle, ar->inode->i_sb,
						     bh, PXT4_JTR_NONE);
		if (err) {
			unlock_buffer(bh);
			goto failed;
...
failed:
	...
	for (; i >= 0; i--) {
		 ...
		 * existing before pxt4_alloc_branch() was called). Also
		 * because blocks are freshly allocated, we don't need to
		 * revoke them which is why we don't set
		 * PXT4_FREE_BLOCKS_METADATA.
		 */
		pxt4_free_blocks(handle, ar->inode, branch[i+1].bh,
				 new_blocks[i], 1,
				 branch[i+1].bh ? PXT4_FREE_BLOCKS_FORGET : 0);
	}
	return err;
}
			      ...
			      Indirect *where, int num)
{
	...
	if (where->bh) {
		...
		err = pxt4_journal_get_write_access(handle, ar->inode->i_sb,
						    where->bh, PXT4_JTR_NONE);
		if (err)
			goto err_out;
	}
	/* That's it */
	...
	*where->p = where->key;
	...
	if (where->bh) {
		 ...
		 */
		pxt4_debug("splicing indirect only\n");
		 ...
		 */
		err = pxt4_mark_inode_dirty(handle, ar->inode);
		if (unlikely(err))
			goto err_out;
		pxt4_debug("splicing direct\n");
	}
	return err;
			...
			int flags)
{
	...
	ASSERT(!(pxt4_test_inode_flag(inode, PXT4_INODE_EXTENTS)));
	ASSERT(handle != NULL || (flags & PXT4_GET_BLOCKS_CREATE) == 0);
	...
	*/
	 ...
	 */
	if (flags & PXT4_GET_BLOCKS_DELALLOC_RESERVE)
		pxt4_da_update_reserve_space(inode, count, 1);
...
static int pxt4_ind_trunc_restart_fn(handle_t *handle, struct inode *inode,
 ...
 */
static int pxt4_ind_truncate_ensure_credits(handle_t *handle,
					    struct inode *inode,
					    struct buffer_head *bh,
					    int revoke_creds)
{
	...
	ret = pxt4_journal_ensure_credits_fn(handle, PXT4_RESERVE_TRANS_BLOCKS,
			pxt4_blocks_for_truncate(inode), revoke_creds,
			pxt4_ind_trunc_restart_fn(handle, inode, bh, &dropped));
	if (dropped)
			     ...
			     __le32 *last)
{
	...
	if (!pxt4_inode_block_valid(inode, block_to_free, count)) {
	...
	err = pxt4_ind_truncate_ensure_credits(handle, inode, bh,
				pxt4_free_data_revoke_credits(inode, count));
	if (err < 0)
		goto out_err;
			       ...
			       __le32 *first, __le32 *last, int depth)
{
	...
	if (depth--) {
		...
		while (--p >= first) {
			...
			if (!pxt4_inode_block_valid(inode, nr, 1)) {
				PXT4_ERROR_INODE(inode,
			...
			bh = pxt4_sb_bread(inode->i_sb, nr, 0);
			 ...
			 */
			if (IS_ERR(bh)) {
				pxt4_error_inode_block(inode, nr, -PTR_ERR(bh),
						       "Read failure");
				continue;
			}
			...
			if (pxt4_handle_is_aborted(handle))
				return;
			if (pxt4_ind_truncate_ensure_credits(handle, inode,
					NULL,
					pxt4_free_metadata_revoke_credits(
							inode->i_sb, 1)) < 0)
				return;
