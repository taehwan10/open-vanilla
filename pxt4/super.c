...
static int pxt4_commit_super(struct super_block *sb);
...
static int pxt4_validate_options(struct fs_context *fc);
static int pxt4_check_opt_consistency(struct fs_context *fc,
				      struct super_block *sb);
static void pxt4_apply_options(struct fs_context *fc, struct super_block *sb);
static int pxt4_parse_param(struct fs_context *fc, struct fs_parameter *param);
static int pxt4_get_tree(struct fs_context *fc);
static int pxt4_reconfigure(struct fs_context *fc);
static void pxt4_fc_free(struct fs_context *fc);
static int pxt4_init_fs_context(struct fs_context *fc);
static void pxt4_kill_sb(struct super_block *sb);
static const struct fs_parameter_spec pxt4_param_specs[];
...
static const struct fs_context_operations pxt4_context_ops = {
	.parse_param	= pxt4_parse_param,
	.get_tree	= pxt4_get_tree,
	.reconfigure	= pxt4_reconfigure,
	.free		= pxt4_fc_free,
};
...
static struct file_system_type pxt2_fs_type = {
	.owner			= THIS_MODULE,
	.name			= "pxt2",
	.init_fs_context	= pxt4_init_fs_context,
	.parameters		= pxt4_param_specs,
	.kill_sb		= pxt4_kill_sb,
	.fs_flags		= FS_REQUIRES_DEV,
};
MODULE_ALIAS_FS("pxt2");
MODULE_ALIAS("pxt2");
#define IS_PXT2_SB(sb) ((sb)->s_type == &pxt2_fs_type)
#else
...
static inline void __pxt4_read_bh(struct buffer_head *bh, blk_opf_t op_flags,
				  bh_end_io_t *end_io)
{
	/*
...
void pxt4_read_bh_nowait(struct buffer_head *bh, blk_opf_t op_flags,
			 bh_end_io_t *end_io)
{
	...
	__pxt4_read_bh(bh, op_flags, end_io);
}

int pxt4_read_bh(struct buffer_head *bh, blk_opf_t op_flags, bh_end_io_t *end_io)
{
	BUG_ON(!buffer_locked(bh));
...
int pxt4_read_bh_lock(struct buffer_head *bh, blk_opf_t op_flags, bool wait)
{
	lock_buffer(bh);
	if (!wait) {
		pxt4_read_bh_nowait(bh, op_flags, NULL);
		return 0;
	}
	return pxt4_read_bh(bh, op_flags, NULL);
}
 ...
 * return.
 */
static struct buffer_head *__pxt4_sb_bread_gfp(struct super_block *sb,
					       sector_t block,
					       blk_opf_t op_flags, gfp_t gfp)
{
	...
	ret = pxt4_read_bh_lock(bh, REQ_META | op_flags, true);
	if (ret) {
		put_bh(bh);
		return ERR_PTR(ret);
	}
...
struct buffer_head *pxt4_sb_bread(struct super_block *sb, sector_t block,
				   blk_opf_t op_flags)
{
	return __pxt4_sb_bread_gfp(sb, block, op_flags, __GFP_MOVABLE);
}

struct buffer_head *pxt4_sb_bread_unmovable(struct super_block *sb,
					    sector_t block)
{
	return __pxt4_sb_bread_gfp(sb, block, 0, 0);
}

void pxt4_sb_breadahead_unmovable(struct super_block *sb, sector_t block)
{
	struct buffer_head *bh = sb_getblk_gfp(sb, block, 0);

	if (likely(bh)) {
		if (trylock_buffer(bh))
			pxt4_read_bh_nowait(bh, REQ_RAHEAD, NULL);
		brelse(bh);
	}
}
...
__le32 pxt4_superblock_csum(struct super_block *sb,
			    struct pxt4_super_block *es)
{
...
static void __pxt4_update_tstamp(__le32 *lo, __u8 *hi, time64_t now)
{
...
#define PXT4_SB_REFRESH_INTERVAL_SEC (3600) /* seconds (1 hour) */
#define PXT4_SB_REFRESH_INTERVAL_KB (16384) /* kilobytes (16MB) */

/*
 * The pxt4_maybe_update_superblock() function checks and updates the
 ...
 * @sb: The superblock
 */
static void pxt4_maybe_update_superblock(struct super_block *sb)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	...
	if (sb_rdonly(sb) || !(sb->s_flags & SB_ACTIVE) ||
	    !journal || (journal->j_flags & JBD3_UNMOUNT))
		return;

	now = ktime_get_real_seconds();
	last_update = pxt4_get_tstamp(es, s_wtime);

	if (likely(now - last_update < PXT4_SB_REFRESH_INTERVAL_SEC))
		return;
	...
	if (diff_size > PXT4_SB_REFRESH_INTERVAL_KB)
		schedule_work(&PXT4_SB(sb)->s_sb_upd_work);
}
...
static void pxt4_journal_commit_callback(journal_t *journal, transaction_t *txn)
{
	...
	pxt4_maybe_update_superblock(sb);
 ...
 */
static int pxt4_journalled_writepage_callback(struct folio *folio,
...
static int pxt4_journalled_submit_inode_data_buffers(struct jbd3_inode *jinode)
{
	...
	return write_cache_pages(mapping, &wbc,
				 pxt4_journalled_writepage_callback,
				 jinode->i_transaction);
}

static int pxt4_journal_submit_inode_data_buffers(struct jbd3_inode *jinode)
{
	int ret;

	if (pxt4_should_journal_data(jinode->i_vfs_inode))
		ret = pxt4_journalled_submit_inode_data_buffers(jinode);
	else
		ret = pxt4_normal_submit_inode_data_buffers(jinode);
	return ret;
}

static int pxt4_journal_finish_inode_data_buffers(struct jbd3_inode *jinode)
{
	int ret = 0;

	if (!pxt4_should_journal_data(jinode->i_vfs_inode))
		ret = jbd3_journal_finish_inode_data_buffers(jinode);
...
struct pxt4_err_translation {
	int code;
	int errno;
};

#define PXT4_ERR_TRANSLATE(err) { .code = PXT4_ERR_##err, .errno = err }

static struct pxt4_err_translation err_translation[] = {
	PXT4_ERR_TRANSLATE(EIO),
	PXT4_ERR_TRANSLATE(ENOMEM),
	PXT4_ERR_TRANSLATE(EFSBADCRC),
	PXT4_ERR_TRANSLATE(EFSCORRUPTED),
	PXT4_ERR_TRANSLATE(ENOSPC),
	PXT4_ERR_TRANSLATE(ENOKEY),
	PXT4_ERR_TRANSLATE(EROFS),
	PXT4_ERR_TRANSLATE(EFBIG),
	PXT4_ERR_TRANSLATE(EEXIST),
	PXT4_ERR_TRANSLATE(ERANGE),
	PXT4_ERR_TRANSLATE(EOVERFLOW),
	PXT4_ERR_TRANSLATE(EBUSY),
	PXT4_ERR_TRANSLATE(ENOTDIR),
	PXT4_ERR_TRANSLATE(ENOTEMPTY),
	PXT4_ERR_TRANSLATE(ESHUTDOWN),
	PXT4_ERR_TRANSLATE(EFAULT),
};

static int pxt4_errno_to_code(int errno)
{
	int i;
 ...
 */
static void pxt4_handle_error(struct super_block *sb, bool force_ro, int error,
			      __u32 ino, __u64 block,
			      const char *func, unsigned int line)
{
	...
	if (!continue_fs && !sb_rdonly(sb)) {
		set_bit(PXT4_FLAGS_SHUTDOWN, &PXT4_SB(sb)->s_pxt4_flags);
		if (journal)
...
{
	struct pxt4_sb_info *sbi = container_of(work, struct pxt4_sb_info,
						s_sb_upd_work);
	journal_t *journal = sbi->s_journal;
	handle_t *handle;
	 ...
	 * updates.
	 *
	 * We use directly jbd3 functions here to avoid recursing back into
	 * pxt4 error handling code during handling of previous errors.
	 */
	if (!sb_rdonly(sbi->s_sb) && journal) {
		struct buffer_head *sbh = sbi->s_sbh;
		bool call_notify_err = false;

		handle = jbd3_journal_start(journal, 1);
		if (IS_ERR(handle))
			goto write_directly;
		if (jbd3_journal_get_write_access(handle, sbh)) {
		...
		if (jbd3_journal_dirty_metadata(handle, sbh)) {
			jbd3_journal_stop(handle);
			goto write_directly;
		}
		jbd3_journal_stop(handle);
		  ...
		  const char *fmt, ...)
{
	...
	pxt4_handle_error(sb, force_ro, error, 0, block, function, line);
}

void __pxt4_error_inode(struct inode *inode, const char *function,
			unsigned int line, pxt4_fsblk_t block, int error,
			const char *fmt, ...)
{
	va_list args;
	struct va_format vaf;
	...
	pxt4_handle_error(inode->i_sb, false, error, inode->i_ino, block,
			  function, line);
}
		       ...
		       const char *fmt, ...)
{
	...
	pxt4_handle_error(inode->i_sb, false, EFSCORRUPTED, inode->i_ino, block,
			  function, line);
}
		      ...
		      unsigned int line, int errno)
{
	...
	pxt4_handle_error(sb, false, -errno, 0, 0, function, line);
}
		...
		const char *prefix, const char *fmt, ...)
{
	...
	if (sb) {
		atomic_inc(&PXT4_SB(sb)->s_msg_count);
		if (!___ratelimit(&(PXT4_SB(sb)->s_msg_ratelimit_state),
				  "PXT4-fs"))
			return;
	}
...
static int pxt4_warning_ratelimit(struct super_block *sb)
{
	atomic_inc(&PXT4_SB(sb)->s_warning_count);
	return ___ratelimit(&(PXT4_SB(sb)->s_warning_ratelimit_state),
			    "PXT4-fs warning");
}
...
__acquires(bitlock)
{
	...
	pxt4_handle_error(sb, false, EFSCORRUPTED, ino, block, function, line);
	/*
...
static inline void pxt4_quotas_off(struct super_block *sb, int type)
{
	BUG_ON(type > PXT4_MAXQUOTAS);
...
static int pxt4_percpu_param_init(struct pxt4_sb_info *sbi)
{
	pxt4_fsblk_t block;
	int err;

	block = pxt4_count_free_clusters(sbi->s_sb);
	pxt4_free_blocks_count_set(sbi->s_es, PXT4_C2B(sbi, block));
	err = percpu_counter_init(&sbi->s_freeclusters_counter, block,
				  GFP_KERNEL);
	if (!err) {
		unsigned long freei = pxt4_count_free_inodes(sbi->s_sb);
	...
	if (!err)
		err = percpu_counter_init(&sbi->s_dirs_counter,
					  pxt4_count_dirs(sbi->s_sb), GFP_KERNEL);
	if (!err)
	...
	if (err)
		pxt4_msg(sbi->s_sb, KERN_ERR, "insufficient memory");

	return err;
}

static void pxt4_percpu_param_destroy(struct pxt4_sb_info *sbi)
{
...
static void pxt4_group_desc_free(struct pxt4_sb_info *sbi)
{
...
static void pxt4_flex_groups_free(struct pxt4_sb_info *sbi)
{
...
static void pxt4_put_super(struct super_block *sb)
{
	...
	/*
	 * Unregister sysfs before destroying jbd3 journal.
	 * Since we could still access attr_journal_task attribute via sysfs
	 * path which could have sbi->s_journal->j_task as NULL
	 * Unregister sysfs before flush sbi->s_sb_upd_work.
	 * Since user may read /proc/fs/pxt4/xx/mb_groups during umount, If
	...
	if (___ratelimit(&pxt4_mount_msg_ratelimit, "PXT4-fs unmount"))
		pxt4_msg(sb, KERN_INFO, "unmounting filesystem %pU.",
			 &sb->s_uuid);

	pxt4_unregister_li_request(sb);
	pxt4_quotas_off(sb, PXT4_MAXQUOTAS);

	flush_work(&sbi->s_sb_upd_work);
	destroy_workqueue(sbi->rsv_conversion_wq);
	pxt4_release_orphan_info(sb);
	...
	if (sbi->s_journal) {
		...
		sbi->s_journal = NULL;
		if ((err < 0) && !aborted) {
			pxt4_abort(sb, -err, "Couldn't clean up the journal");
		}
	}
	...
	if (!sb_rdonly(sb) && !aborted) {
		...
		pxt4_clear_feature_orphan_present(sb);
		es->s_state = cpu_to_le16(sbi->s_mount_state);
	}
	if (!sb_rdonly(sb))
		pxt4_commit_super(sb);

	pxt4_group_desc_free(sbi);
	pxt4_flex_groups_free(sbi);
	pxt4_percpu_param_destroy(sbi);
#ifdef CONFIG_QUOTA
	for (int i = 0; i < PXT4_MAXQUOTAS; i++)
...
static struct inode *pxt4_alloc_inode(struct super_block *sb)
{
	...
	ei = alloc_inode_sb(sb, pxt4_inode_cachep, GFP_NOFS);
	if (!ei)
		return NULL;
...
static void pxt4_shutdown(struct super_block *sb)
{
...
static const struct constant_table pxt4_param_errors[] = {
	{"continue",	PXT4_MOUNT_ERRORS_CONT},
	{"panic",	PXT4_MOUNT_ERRORS_PANIC},
	{"remount-ro",	PXT4_MOUNT_ERRORS_RO},
	{}
};

static const struct constant_table pxt4_param_data[] = {
	{"journal",	PXT4_MOUNT_JOURNAL_DATA},
	{"ordered",	PXT4_MOUNT_ORDERED_DATA},
	{"writeback",	PXT4_MOUNT_WRITEBACK_DATA},
	{}
};

static const struct constant_table pxt4_param_data_err[] = {
	{"abort",	Opt_data_err_abort},
	{"ignore",	Opt_data_err_ignore},
	{}
};

static const struct constant_table pxt4_param_jqfmt[] = {
	{"vfsold",	QFMT_VFS_OLD},
	{"vfsv0",	QFMT_VFS_V0},
	{"vfsv1",	QFMT_VFS_V1},
	{}
...
static const struct constant_table pxt4_param_dax[] = {
 ...
 * options and the way we show them in _pxt4_show_options(). To
...
static const struct fs_parameter_spec pxt4_param_specs[] = {
	...
	fsparam_enum	("errors",		Opt_errors, pxt4_param_errors),
	...
	fsparam_enum	("data",		Opt_data, pxt4_param_data),
	...
	fsparam_enum	("jqfmt",		Opt_jqfmt, pxt4_param_jqfmt),
	...
	fsparam_enum	("dax",			Opt_dax_type, pxt4_param_dax),
	...
	fsparam_string	("check",		Opt_removed),	/* mount option from pxt2/3 */
	fsparam_flag	("nocheck",		Opt_removed),	/* mount option from pxt2/3 */
	fsparam_flag	("reservation",		Opt_removed),	/* mount option from pxt2/3 */
	fsparam_flag	("noreservation",	Opt_removed),	/* mount option from pxt2/3 */
	fsparam_u32	("journal",		Opt_removed),	/* mount option from pxt2/3 */
	{}
};
...
static const struct mount_opts {
	...
	{Opt_data_err, PXT4_MOUNT_DATA_ERR_ABORT, MOPT_NO_PXT2},
	...
	{Opt_dax_type, 0, MOPT_PXT4_ONLY},
	{Opt_journal_dev, 0, MOPT_NO_PXT2},
	{Opt_journal_path, 0, MOPT_NO_PXT2},
	{Opt_journal_ioprio, 0, MOPT_NO_PXT2},
	{Opt_data, 0, MOPT_NO_PXT2},
	...
	{Opt_no_prefetch_block_bitmaps, PXT4_MOUNT_NO_PREFETCH_BLOCK_BITMAPS,
	 MOPT_SET},
#ifdef CONFIG_PXT4_DEBUG
	{Opt_fc_debug_force, PXT4_MOUNT2_JOURNAL_FAST_COMMIT,
	 MOPT_SET | MOPT_2 | MOPT_PXT4_ONLY},
#endif
	{Opt_abort, PXT4_MOUNT2_ABORT, MOPT_SET | MOPT_2},
	{Opt_err, 0, 0}
};
...
static const struct pxt4_sb_encodings {
	...
	{PXT4_ENC_UTF8_12_1, "utf8", UNICODE_AGE(12, 1, 0)},
};

static const struct pxt4_sb_encodings *
pxt4_sb_read_encoding(const struct pxt4_super_block *es)
{
	...
	for (i = 0; i < ARRAY_SIZE(pxt4_sb_encoding_map); i++)
		if (magic == pxt4_sb_encoding_map[i].magic)
			return &pxt4_sb_encoding_map[i];

	return NULL;
}
#endif

#define PXT4_SPEC_JQUOTA			(1 <<  0)
#define PXT4_SPEC_JQFMT				(1 <<  1)
#define PXT4_SPEC_DATAJ				(1 <<  2)
#define PXT4_SPEC_SB_BLOCK			(1 <<  3)
#define PXT4_SPEC_JOURNAL_DEV			(1 <<  4)
#define PXT4_SPEC_JOURNAL_IOPRIO		(1 <<  5)
#define PXT4_SPEC_s_want_extra_isize		(1 <<  7)
#define PXT4_SPEC_s_max_batch_time		(1 <<  8)
#define PXT4_SPEC_s_min_batch_time		(1 <<  9)
#define PXT4_SPEC_s_inode_readahead_blks	(1 << 10)
#define PXT4_SPEC_s_li_wait_mult		(1 << 11)
#define PXT4_SPEC_s_max_dir_size_kb		(1 << 12)
#define PXT4_SPEC_s_stripe			(1 << 13)
#define PXT4_SPEC_s_resuid			(1 << 14)
#define PXT4_SPEC_s_resgid			(1 << 15)
#define PXT4_SPEC_s_commit_interval		(1 << 16)
#define PXT4_SPEC_s_fc_debug_max_replay		(1 << 17)
#define PXT4_SPEC_s_sb_block			(1 << 18)
#define PXT4_SPEC_mb_optimize_scan		(1 << 19)

struct pxt4_fs_context {
...
int pxt4_init_fs_context(struct fs_context *fc)
{
	struct pxt4_fs_context *ctx;

	ctx = kzalloc(sizeof(struct pxt4_fs_context), GFP_KERNEL);
	if (!ctx)
		return -ENOMEM;

	fc->fs_private = ctx;
	fc->ops = &pxt4_context_ops;
		       ...
		       struct fs_parameter *param)
{
	...
	if (param->size < 1) {
		pxt4_msg(NULL, KERN_ERR, "Missing quota name");
		return -EINVAL;
	}
	if (strchr(param->string, '/')) {
		pxt4_msg(NULL, KERN_ERR,
...
static int pxt4_parse_test_dummy_encryption(const struct fs_parameter *param,
					    struct pxt4_fs_context *ctx)
{
	int err;

	if (!IS_ENABLED(CONFIG_FS_ENCRYPTION)) {
		pxt4_msg(NULL, KERN_WARNING,
...
#define PXT4_SET_CTX(name)						\
static inline void ctx_set_##name(struct pxt4_fs_context *ctx,		\
				  unsigned long flag)			\
{									\
...
#define PXT4_CLEAR_CTX(name)						\
static inline void ctx_clear_##name(struct pxt4_fs_context *ctx,	\
				    unsigned long flag)			\
{									\
	ctx->mask_s_##name |= flag;					\
	ctx->vals_s_##name &= ~flag;					\
...
#define PXT4_TEST_CTX(name)						\
static inline unsigned long						\
ctx_test_##name(struct pxt4_fs_context *ctx, unsigned long flag)	\
{									\
	return (ctx->vals_s_##name & flag);				\
}

PXT4_SET_CTX(flags); /* set only */
PXT4_SET_CTX(mount_opt);
PXT4_CLEAR_CTX(mount_opt);
PXT4_TEST_CTX(mount_opt);
PXT4_SET_CTX(mount_opt2);
PXT4_CLEAR_CTX(mount_opt2);
PXT4_TEST_CTX(mount_opt2);
...
static int pxt4_parse_param(struct fs_context *fc, struct fs_parameter *param)
{
	...
	token = fs_parse(fc, pxt4_param_specs, param, &result);
	if (token < 0)
		return token;
	...
	if (m->flags & MOPT_EXPLICIT) {
		if (m->mount_opt & PXT4_MOUNT_DELALLOC) {
			ctx_set_mount_opt2(ctx, PXT4_MOUNT2_EXPLICIT_DELALLOC);
	...
	if (m->flags & MOPT_NOSUPPORT) {
		pxt4_msg(NULL, KERN_ERR, "%s option not supported",
			 param->key);
		return 0;
	}
	...
	case Opt_sb:
		if (fc->purpose == FS_CONTEXT_FOR_RECONFIGURE) {
			...
			ctx->spec |= PXT4_SPEC_s_sb_block;
		}
		return 0;
	case Opt_removed:
		pxt4_msg(NULL, KERN_WARNING, "Ignoring removed %s option",
			 param->key);
		return 0;
...
#else
		pxt4_msg(NULL, KERN_ERR, "inline encryption not supported");
#endif
		return 0;
	case Opt_errors:
		ctx_clear_mount_opt(ctx, PXT4_MOUNT_ERRORS_MASK);
		ctx_set_mount_opt(ctx, result.uint_32);
		return 0;
#ifdef CONFIG_QUOTA
	case Opt_jqfmt:
		ctx->s_jquota_fmt = result.uint_32;
		ctx->spec |= PXT4_SPEC_JQFMT;
		return 0;
#endif
	case Opt_data:
		ctx_clear_mount_opt(ctx, PXT4_MOUNT_DATA_FLAGS);
		ctx_set_mount_opt(ctx, result.uint_32);
		ctx->spec |= PXT4_SPEC_DATAJ;
		return 0;
	case Opt_commit:
		if (result.uint_32 == 0)
			result.uint_32 = JBD3_DEFAULT_MAX_COMMIT_AGE;
		...
		ctx->spec |= PXT4_SPEC_s_commit_interval;
		return 0;
	case Opt_debug_want_extra_isize:
		...
		ctx->spec |= PXT4_SPEC_s_want_extra_isize;
		return 0;
	case Opt_max_batch_time:
		ctx->s_max_batch_time = result.uint_32;
		ctx->spec |= PXT4_SPEC_s_max_batch_time;
		return 0;
	case Opt_min_batch_time:
		ctx->s_min_batch_time = result.uint_32;
		ctx->spec |= PXT4_SPEC_s_min_batch_time;
		return 0;
	case Opt_inode_readahead_blks:
		...
		ctx->spec |= PXT4_SPEC_s_inode_readahead_blks;
		return 0;
	case Opt_init_itable:
		ctx_set_mount_opt(ctx, PXT4_MOUNT_INIT_INODE_TABLE);
		ctx->s_li_wait_mult = PXT4_DEF_LI_WAIT_MULT;
		if (param->type == fs_value_is_string)
			ctx->s_li_wait_mult = result.uint_32;
		ctx->spec |= PXT4_SPEC_s_li_wait_mult;
		return 0;
	case Opt_max_dir_size_kb:
		ctx->s_max_dir_size_kb = result.uint_32;
		ctx->spec |= PXT4_SPEC_s_max_dir_size_kb;
		return 0;
#ifdef CONFIG_PXT4_DEBUG
	case Opt_fc_debug_max_replay:
		ctx->s_fc_debug_max_replay = result.uint_32;
		ctx->spec |= PXT4_SPEC_s_fc_debug_max_replay;
		return 0;
#endif
	case Opt_stripe:
		ctx->s_stripe = result.uint_32;
		ctx->spec |= PXT4_SPEC_s_stripe;
		return 0;
	case Opt_resuid:
		uid = make_kuid(current_user_ns(), result.uint_32);
		if (!uid_valid(uid)) {
			pxt4_msg(NULL, KERN_ERR, "Invalid uid value %d",
				 result.uint_32);
			return -EINVAL;
		}
		ctx->s_resuid = uid;
		ctx->spec |= PXT4_SPEC_s_resuid;
		return 0;
	case Opt_resgid:
		gid = make_kgid(current_user_ns(), result.uint_32);
		if (!gid_valid(gid)) {
			pxt4_msg(NULL, KERN_ERR, "Invalid gid value %d",
				 result.uint_32);
			return -EINVAL;
		}
		ctx->s_resgid = gid;
		ctx->spec |= PXT4_SPEC_s_resgid;
		return 0;
	case Opt_journal_dev:
		...
		ctx->spec |= PXT4_SPEC_JOURNAL_DEV;
		return 0;
	case Opt_journal_path:
	{
		...
		if (error) {
			pxt4_msg(NULL, KERN_ERR, "error: could not find "
				 "journal device path");
			return -EINVAL;
		}
	...
	case Opt_journal_ioprio:
		if (result.uint_32 > 7) {
			pxt4_msg(NULL, KERN_ERR, "Invalid journal IO priority"
				 " (must be 0-7)");
			return -EINVAL;
		}
		ctx->journal_ioprio =
			IOPRIO_PRIO_VALUE(IOPRIO_CLASS_BE, result.uint_32);
		ctx->spec |= PXT4_SPEC_JOURNAL_IOPRIO;
		return 0;
	case Opt_test_dummy_encryption:
		return pxt4_parse_test_dummy_encryption(param, ctx);
	case Opt_dax:
	case Opt_dax_type:
#ifdef CONFIG_FS_DAX
	{
		...
		case Opt_dax:
		case Opt_dax_always:
			ctx_set_mount_opt(ctx, PXT4_MOUNT_DAX_ALWAYS);
			ctx_clear_mount_opt2(ctx, PXT4_MOUNT2_DAX_NEVER);
			break;
		case Opt_dax_never:
			ctx_set_mount_opt2(ctx, PXT4_MOUNT2_DAX_NEVER);
			ctx_clear_mount_opt(ctx, PXT4_MOUNT_DAX_ALWAYS);
			break;
		case Opt_dax_inode:
			ctx_clear_mount_opt(ctx, PXT4_MOUNT_DAX_ALWAYS);
			ctx_clear_mount_opt2(ctx, PXT4_MOUNT2_DAX_NEVER);
			/* Strictly for printing options */
			ctx_set_mount_opt2(ctx, PXT4_MOUNT2_DAX_INODE);
			break;
		}
		return 0;
	}
#else
		pxt4_msg(NULL, KERN_INFO, "dax option not supported");
		return -EINVAL;
#endif
	case Opt_data_err:
	...
	case Opt_mb_optimize_scan:
		if (result.int_32 == 1) {
			ctx_set_mount_opt2(ctx, PXT4_MOUNT2_MB_OPTIMIZE_SCAN);
			ctx->spec |= PXT4_SPEC_mb_optimize_scan;
		} else if (result.int_32 == 0) {
			ctx_clear_mount_opt2(ctx, PXT4_MOUNT2_MB_OPTIMIZE_SCAN);
			ctx->spec |= PXT4_SPEC_mb_optimize_scan;
		} else {
			pxt4_msg(NULL, KERN_WARNING,
	 ...
	 */
	if (m->token == Opt_err) {
		pxt4_msg(NULL, KERN_WARNING, "buggy handling of option %s",
			 param->key);
		WARN_ON(1);
		return -EINVAL;
	}
...
static int parse_apply_sb_mount_options(struct super_block *sb,
					struct pxt4_fs_context *m_ctx)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	char *s_mount_opts = NULL;
	struct pxt4_fs_context *s_ctx = NULL;
	...
	s_ctx = kzalloc(sizeof(struct pxt4_fs_context), GFP_KERNEL);
	if (!s_ctx)
		goto out_free;
	...
	ret = pxt4_check_opt_consistency(fc, sb);
	if (ret < 0) {
parse_failed:
	...
	if (s_ctx->spec & PXT4_SPEC_JOURNAL_DEV)
		m_ctx->journal_devnum = s_ctx->journal_devnum;
	if (s_ctx->spec & PXT4_SPEC_JOURNAL_IOPRIO)
		m_ctx->journal_ioprio = s_ctx->journal_ioprio;

	pxt4_apply_options(fc, sb);
	ret = 0;
...
static void pxt4_apply_quota_options(struct fs_context *fc,
				     struct super_block *sb)
{
#ifdef CONFIG_QUOTA
	bool quota_feature = pxt4_has_feature_quota(sb);
	struct pxt4_fs_context *ctx = fc->fs_private;
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	char *qname;
	int i;
 ...
 */
static int pxt4_check_quota_consistency(struct fs_context *fc,
					struct super_block *sb)
{
#ifdef CONFIG_QUOTA
	struct pxt4_fs_context *ctx = fc->fs_private;
	 ...
	 */
	if (ctx_test_mount_opt(ctx, PXT4_MOUNT_PRJQUOTA) &&
	    !pxt4_has_feature_project(sb)) {
		pxt4_msg(NULL, KERN_ERR, "Project quota feature not enabled. "
			 "Cannot enable project quota enforcement.");
		return -EINVAL;
	}

	quota_flags = PXT4_MOUNT_QUOTA | PXT4_MOUNT_USRQUOTA |
		      PXT4_MOUNT_GRPQUOTA | PXT4_MOUNT_PRJQUOTA;
	if (quota_loaded &&
	...
	if (ctx->spec & PXT4_SPEC_JQFMT) {
		if (sbi->s_jquota_fmt != ctx->s_jquota_fmt && quota_loaded)
			goto err_jquota_change;
		if (quota_feature) {
			pxt4_msg(NULL, KERN_INFO, "Quota format mount options "
	...
	usrquota = (ctx_test_mount_opt(ctx, PXT4_MOUNT_USRQUOTA) ||
		    test_opt(sb, USRQUOTA));

	grpquota = (ctx_test_mount_opt(ctx, PXT4_MOUNT_GRPQUOTA) ||
		    test_opt(sb, GRPQUOTA));

	if (usr_qf_name) {
		ctx_clear_mount_opt(ctx, PXT4_MOUNT_USRQUOTA);
		usrquota = false;
	}
	if (grp_qf_name) {
		ctx_clear_mount_opt(ctx, PXT4_MOUNT_GRPQUOTA);
		grpquota = false;
	}

	if (usr_qf_name || grp_qf_name) {
		if (usrquota || grpquota) {
			pxt4_msg(NULL, KERN_ERR, "old and new quota "
				 "format mixing");
			return -EINVAL;
		}

		if (!(ctx->spec & PXT4_SPEC_JQFMT || sbi->s_jquota_fmt)) {
			pxt4_msg(NULL, KERN_ERR, "journaled quota format "
				 "not specified");
			return -EINVAL;
		}
	}
...
err_quota_change:
	...
	return -EINVAL;
err_jquota_change:
	pxt4_msg(NULL, KERN_ERR, "Cannot change journaled quota "
		 "options when quota turned on");
	return -EINVAL;
err_jquota_specified:
	pxt4_msg(NULL, KERN_ERR, "%s quota file already specified",
		 QTYPE2NAME(i));
	return -EINVAL;
#else
	return 0;
...
static int pxt4_check_test_dummy_encryption(const struct fs_context *fc,
					    struct super_block *sb)
{
	const struct pxt4_fs_context *ctx = fc->fs_private;
	const struct pxt4_sb_info *sbi = PXT4_SB(sb);
...
static void pxt4_apply_test_dummy_encryption(struct pxt4_fs_context *ctx,
					     struct super_block *sb)
{
	if (!fscrypt_is_dummy_policy_set(&ctx->dummy_enc_policy) ||
	    /* if already set, it was already verified to be the same */
	    fscrypt_is_dummy_policy_set(&PXT4_SB(sb)->s_dummy_enc_policy))
		return;
	PXT4_SB(sb)->s_dummy_enc_policy = ctx->dummy_enc_policy;
	memset(&ctx->dummy_enc_policy, 0, sizeof(ctx->dummy_enc_policy));
	pxt4_msg(sb, KERN_WARNING, "Test dummy encryption mode enabled");
}
...
{
	struct pxt4_fs_context *ctx = fc->fs_private;
	struct pxt4_sb_info *sbi = fc->s_fs_info;
	int is_remount = fc->purpose == FS_CONTEXT_FOR_RECONFIGURE;
	int err;

	if ((ctx->opt_flags & MOPT_NO_PXT2) && IS_PXT2_SB(sb)) {
		pxt4_msg(NULL, KERN_ERR,
			 "Mount option(s) incompatible with pxt2");
		return -EINVAL;
	}
	...
	if (ctx_test_mount_opt(ctx, PXT4_MOUNT_DIOREAD_NOLOCK)) {
		int blocksize =
			BLOCK_SIZE << le32_to_cpu(sbi->s_es->s_log_block_size);
		if (blocksize < PAGE_SIZE)
			pxt4_msg(NULL, KERN_WARNING, "Warning: mounting with an "
				 "experimental mount option 'dioread_nolock' "
				 "for blocksize < PAGE_SIZE");
	}

	err = pxt4_check_test_dummy_encryption(fc, sb);
	if (err)
		return err;

	if ((ctx->spec & PXT4_SPEC_DATAJ) && is_remount) {
		if (!sbi->s_journal) {
			pxt4_msg(NULL, KERN_WARNING,
				 "Remounting file system with no journal "
				 "so ignoring journalled data option");
			ctx_clear_mount_opt(ctx, PXT4_MOUNT_DATA_FLAGS);
		} else if (ctx_test_mount_opt(ctx, PXT4_MOUNT_DATA_FLAGS) !=
			   test_opt(sb, DATA_FLAGS)) {
			pxt4_msg(NULL, KERN_ERR, "Cannot change data mode "
				 "on remount");
			return -EINVAL;
		}
	}
	...
	if (is_remount) {
		if (ctx_test_mount_opt(ctx, PXT4_MOUNT_DAX_ALWAYS) &&
		    (test_opt(sb, DATA_FLAGS) == PXT4_MOUNT_JOURNAL_DATA)) {
			pxt4_msg(NULL, KERN_ERR, "can't mount with "
				 "both data=journal and dax");
			return -EINVAL;
		}

		if (ctx_test_mount_opt(ctx, PXT4_MOUNT_DAX_ALWAYS) &&
		    (!(sbi->s_mount_opt & PXT4_MOUNT_DAX_ALWAYS) ||
		     (sbi->s_mount_opt2 & PXT4_MOUNT2_DAX_NEVER))) {
fail_dax_change_remount:
			pxt4_msg(NULL, KERN_ERR, "can't change "
				 "dax mount option while remounting");
			return -EINVAL;
		} else if (ctx_test_mount_opt2(ctx, PXT4_MOUNT2_DAX_NEVER) &&
			 (!(sbi->s_mount_opt2 & PXT4_MOUNT2_DAX_NEVER) ||
			  (sbi->s_mount_opt & PXT4_MOUNT_DAX_ALWAYS))) {
			goto fail_dax_change_remount;
		} else if (ctx_test_mount_opt2(ctx, PXT4_MOUNT2_DAX_INODE) &&
			   ((sbi->s_mount_opt & PXT4_MOUNT_DAX_ALWAYS) ||
			    (sbi->s_mount_opt2 & PXT4_MOUNT2_DAX_NEVER) ||
			    !(sbi->s_mount_opt2 & PXT4_MOUNT2_DAX_INODE))) {
			goto fail_dax_change_remount;
		}
	}

	return pxt4_check_quota_consistency(fc, sb);
}
...
#define APPLY(X) ({ if (ctx->spec & PXT4_SPEC_##X) sbi->X = ctx->X; })
	...
	pxt4_apply_quota_options(fc, sb);
	pxt4_apply_test_dummy_encryption(ctx, sb);
}
...
static int pxt4_validate_options(struct fs_context *fc)
{
	...
	if (usr_qf_name || grp_qf_name) {
		if (ctx_test_mount_opt(ctx, PXT4_MOUNT_USRQUOTA) && usr_qf_name)
			ctx_clear_mount_opt(ctx, PXT4_MOUNT_USRQUOTA);

		if (ctx_test_mount_opt(ctx, PXT4_MOUNT_GRPQUOTA) && grp_qf_name)
			ctx_clear_mount_opt(ctx, PXT4_MOUNT_GRPQUOTA);

		if (ctx_test_mount_opt(ctx, PXT4_MOUNT_USRQUOTA) ||
		    ctx_test_mount_opt(ctx, PXT4_MOUNT_GRPQUOTA)) {
			pxt4_msg(NULL, KERN_ERR, "old and new quota "
				 "format mixing");
			return -EINVAL;
		}
...
static const char *token2str(int token)
{
	...
	for (spec = pxt4_param_specs; spec->name != NULL; spec++)
			    ...
			    int read_only)
{
	...
	if (sbi->s_journal) {
		...
		if (pxt4_has_feature_orphan_file(sb))
			pxt4_set_feature_orphan_present(sb);
	}

	err = pxt4_commit_super(sb);
done:
				   ...
				   struct pxt4_group_desc *gdp)
{
	...
	if (pxt4_has_feature_64bit(sb) && offset < sbi->s_desc_size)
		crc = crc16(crc, (__u8 *)gdp + offset,
			    sbi->s_desc_size - offset);
...
{
	loff_t upper_limit, res = PXT4_NDIR_BLOCKS;
	int meta_blocks;
 ...
 * 0 if it cannot be.
 */
int pxt4_feature_set_ok(struct super_block *sb, int readonly)
{
...
static int pxt4_run_li_request(struct pxt4_li_request *elr)
{
	...
	pxt4_group_t group = elr->lr_next_group;
	unsigned int prefetch_ios = 0;
	int ret = 0;
	int nr = PXT4_SB(sb)->s_mb_prefetch;
	u64 start_time;

	if (elr->lr_mode == PXT4_LI_MODE_PREFETCH_BBITMAP) {
		elr->lr_next_group = pxt4_mb_prefetch(sb, group, nr, &prefetch_ios);
		pxt4_mb_prefetch_fini(sb, elr->lr_next_group, nr);
		trace_pxt4_prefetch_bitmaps(sb, group, elr->lr_next_group, nr);
		if (group >= elr->lr_next_group) {
			ret = 1;
			if (elr->lr_first_not_zeroed != ngroups &&
			    !sb_rdonly(sb) && test_opt(sb, INIT_INODE_TABLE)) {
				elr->lr_next_group = elr->lr_first_not_zeroed;
				elr->lr_mode = PXT4_LI_MODE_ITABLE;
				ret = 0;
			}
		}
		return ret;
	...
	if (!ret) {
		...
		ret = pxt4_init_inode_table(sb, group,
					    elr->lr_timeout ? 0 : 1);
		trace_pxt4_lazy_itable_init(sb, group);
		if (elr->lr_timeout == 0) {
			elr->lr_timeout = nsecs_to_jiffies((ktime_get_real_ns() - start_time) *
				PXT4_SB(elr->lr_super)->s_li_wait_mult);
		}
...
static void pxt4_remove_li_request(struct pxt4_li_request *elr)
{
	...
	PXT4_SB(elr->lr_super)->s_li_request = NULL;
	kfree(elr);
}
		...
		list_for_each_safe(pos, n, &eli->li_request_list) {
			...
			if (!progress) {
				elr->lr_next_sched = jiffies +
					get_random_u32_below(PXT4_DEF_LI_MAX_START_DELAY * HZ);
			}
			if (time_before(elr->lr_next_sched, next_wakeup))
				next_wakeup = elr->lr_next_sched;
		}
					    ...
					    pxt4_group_t start)
{
	...
	if (test_opt(sb, NO_PREFETCH_BLOCK_BITMAPS)) {
		...
		elr->lr_mode = PXT4_LI_MODE_PREFETCH_BBITMAP;
	}
	 ...
	 * better.
	 */
	elr->lr_next_sched = jiffies + get_random_u32_below(PXT4_DEF_LI_MAX_START_DELAY * HZ);
	return elr;
}
			  ...
			  char *buf)
{
	...
	int			s, j, count = 0;
	int			has_super = pxt4_bg_has_super(sb, grp);

	if (!pxt4_has_feature_bigalloc(sb))
		return (has_super + pxt4_bg_num_gdb(sb, grp) +
...
int pxt4_calculate_overhead(struct super_block *sb)
{
	...
	if (sbi->s_journal && !sbi->s_journal_bdev)
		overhead += PXT4_NUM_B2C(sbi, sbi->s_journal->j_total_len);
...
static const char *pxt4_quota_mode(struct super_block *sb)
{
#ifdef CONFIG_QUOTA
	if (!pxt4_quota_capable(sb))
		return "none";

	if (PXT4_SB(sb)->s_journal && pxt4_is_quota_journalled(sb))
		return "journalled";
	else
		return "writeback";
#else
...
static void pxt4_setup_csum_trigger(struct super_block *sb,
				    enum pxt4_journal_trigger_type type,
				    void (*trigger)(
...
static void pxt4_free_sbi(struct pxt4_sb_info *sbi)
{
	if (!sbi)
		return;
...
static struct pxt4_sb_info *pxt4_alloc_sbi(struct super_block *sb)
{
	struct pxt4_sb_info *sbi;
...
static void pxt4_set_def_opts(struct super_block *sb,
			      struct pxt4_super_block *es)
{
...
#endif
	if (pxt4_has_feature_fast_commit(sb))
	...
	if (le16_to_cpu(es->s_errors) == PXT4_ERRORS_PANIC)
		set_opt(sb, ERRORS_PANIC);
	else if (le16_to_cpu(es->s_errors) == PXT4_ERRORS_CONTINUE)
...
static int pxt4_handle_clustersize(struct super_block *sb)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	struct pxt4_super_block *es = sbi->s_es;
	int clustersize;
...
static void pxt4_fast_commit_init(struct super_block *sb)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
...
static int pxt4_inode_info_init(struct super_block *sb,
				struct pxt4_super_block *es)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	...
	if (le32_to_cpu(es->s_rev_level) == PXT4_GOOD_OLD_REV) {
		...
		if ((sbi->s_inode_size < PXT4_GOOD_OLD_INODE_SIZE) ||
			...
			pxt4_msg(sb, KERN_ERR,
			       ...
			       sbi->s_inode_size);
			pxt4_msg(sb, KERN_ERR, "blocksize: %lu", sb->s_blocksize);
			return -EINVAL;
		}
		/*
...
#if IS_ENABLED(CONFIG_UNICODE)
static int pxt4_encoding_init(struct super_block *sb, struct pxt4_super_block *es)
{
	const struct pxt4_sb_encodings *encoding_info;
	struct unicode_map *encoding;
	__u16 encoding_flags = le16_to_cpu(es->s_encoding_flags);

	if (!pxt4_has_feature_casefold(sb) || sb->s_encoding)
		return 0;

	encoding_info = pxt4_sb_read_encoding(es);
	if (!encoding_info) {
		pxt4_msg(sb, KERN_ERR,
...
#else
static inline int pxt4_encoding_init(struct super_block *sb, struct pxt4_super_block *es)
{
	return 0;
}
#endif

static int pxt4_init_metadata_csum(struct super_block *sb, struct pxt4_super_block *es)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	...
	pxt4_setup_csum_trigger(sb, PXT4_JTR_ORPHAN_FILE,
				pxt4_orphan_file_block_trigger);
...
static int pxt4_check_feature_compatibility(struct super_block *sb,
					    struct pxt4_super_block *es,
					    int silent)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	...
	if (sbi->s_daxdev) {
		if (sb->s_blocksize == PAGE_SIZE)
			set_bit(PXT4_FLAGS_BDEV_IS_DAX, &sbi->s_pxt4_flags);
		else
			pxt4_msg(sb, KERN_ERR, "unsupported blocksize for DAX\n");
	}

	if (sbi->s_mount_opt & PXT4_MOUNT_DAX_ALWAYS) {
		if (pxt4_has_feature_inline_data(sb)) {
			pxt4_msg(sb, KERN_ERR, "Cannot use DAX on a filesystem"
					" that may contain inline data");
			return -EINVAL;
		}
		if (!test_bit(PXT4_FLAGS_BDEV_IS_DAX, &sbi->s_pxt4_flags)) {
			pxt4_msg(sb, KERN_ERR,
...
static int pxt4_check_geometry(struct super_block *sb,
			       struct pxt4_super_block *es)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	__u64 blocks_count;
...
static int pxt4_group_desc_init(struct super_block *sb,
				struct pxt4_super_block *es,
				...
				pxt4_group_t *first_not_zeroed)
{
	...
	for (i = 0; i < db_count; i++) {
		...
		pxt4_sb_breadahead_unmovable(sb, block);
	}
	...
	for (i = 0; i < db_count; i++) {
		...
		block = descriptor_loc(sb, logical_sb_block, i);
		bh = pxt4_sb_bread_unmovable(sb, block);
		if (IS_ERR(bh)) {
			pxt4_msg(sb, KERN_ERR,
...
static int pxt4_load_and_init_journal(struct super_block *sb,
				      struct pxt4_super_block *es,
				      struct pxt4_fs_context *ctx)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	...
	err = pxt4_load_journal(sb, es, ctx->journal_devnum);
	if (err)
		return err;
	...
	if (test_opt2(sb, JOURNAL_FAST_COMMIT) &&
		!jbd3_journal_set_features(PXT4_SB(sb)->s_journal, 0, 0,
					  JBD3_FEATURE_INCOMPAT_FAST_COMMIT)) {
		pxt4_msg(sb, KERN_ERR,
	...
	sbi->s_journal->j_submit_inode_data_buffers =
		pxt4_journal_submit_inode_data_buffers;
	sbi->s_journal->j_finish_inode_data_buffers =
		pxt4_journal_finish_inode_data_buffers;
...
static int pxt4_check_journal_data_mode(struct super_block *sb)
{
	if (test_opt(sb, DATA_FLAGS) == PXT4_MOUNT_JOURNAL_DATA) {
		printk_once(KERN_WARNING "PXT4-fs: Warning: mounting with "
...
static int pxt4_load_super(struct super_block *sb, pxt4_fsblk_t *lsb,
			   int silent)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	struct pxt4_super_block *es;
	...
	if (blocksize != PXT4_MIN_BLOCK_SIZE) {
		logical_sb_block = sbi->s_sb_block * PXT4_MIN_BLOCK_SIZE;
		offset = do_div(logical_sb_block, blocksize);
	} else {
		logical_sb_block = sbi->s_sb_block;
	}

	bh = pxt4_sb_bread_unmovable(sb, logical_sb_block);
	if (IS_ERR(bh)) {
	...
	blocksize = PXT4_MIN_BLOCK_SIZE << le32_to_cpu(es->s_log_block_size);
	...
	bh = pxt4_sb_bread_unmovable(sb, logical_sb_block);
	if (IS_ERR(bh)) {
		pxt4_msg(sb, KERN_ERR, "Can't read superblock on 2nd try");
		ret = PTR_ERR(bh);
		bh = NULL;
		goto out;
	}
	...
	if (es->s_magic != cpu_to_le16(PXT4_SUPER_MAGIC)) {
		pxt4_msg(sb, KERN_ERR, "Magic mismatch, very weird!");
		goto out;
	}
	*lsb = logical_sb_block;
	sbi->s_sbh = bh;
...
static void pxt4_hash_info_init(struct super_block *sb)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	struct pxt4_super_block *es = sbi->s_es;
	unsigned int i;
...
static int pxt4_block_group_meta_init(struct super_block *sb, int silent)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	struct pxt4_super_block *es = sbi->s_es;
	int has_huge_files;
	...
	sbi->s_inodes_per_block = sb->s_blocksize / PXT4_INODE_SIZE(sb);
	...
	sbi->s_desc_per_block = sb->s_blocksize / PXT4_DESC_SIZE(sb);
	sbi->s_mount_state = le16_to_cpu(es->s_state) & ~PXT4_FC_REPLAY;
...
static int __pxt4_fill_super(struct fs_context *fc, struct super_block *sb)
{
	...
	if (!(ctx->spec & PXT4_SPEC_JOURNAL_IOPRIO))
	...
	err = pxt4_load_super(sb, &logical_sb_block, silent);
	if (err)
		goto out_fail;
	...
	err = pxt4_init_metadata_csum(sb, es);
	if (err)
		goto failed_mount;

	pxt4_set_def_opts(sb, es);
	...
	err = pxt4_inode_info_init(sb, es);
	if (err)
		goto failed_mount;
	...
	err = pxt4_check_opt_consistency(fc, sb);
	if (err < 0)
		goto failed_mount;

	pxt4_apply_options(fc, sb);

	err = pxt4_encoding_init(sb, es);
	if (err)
		goto failed_mount;

	err = pxt4_check_journal_data_mode(sb);
	if (err)
		goto failed_mount;
	...
	err = pxt4_check_feature_compatibility(sb, es, silent);
	if (err)
		goto failed_mount;

	err = pxt4_block_group_meta_init(sb, silent);
	if (err)
		goto failed_mount;

	pxt4_hash_info_init(sb);

	err = pxt4_handle_clustersize(sb);
	if (err)
		goto failed_mount;

	err = pxt4_check_geometry(sb, es);
	if (err)
		goto failed_mount;
	...
	err = pxt4_group_desc_init(sb, es, logical_sb_block, &first_not_zeroed);
	if (err)
		goto failed_mount3;

	err = pxt4_es_register_shrinker(sbi);
	if (err)
		goto failed_mount3;
	...
	pxt4_fast_commit_init(sb);

	sb->s_root = NULL;

	needs_recovery = (es->s_last_orphan != 0 ||
			  pxt4_has_feature_orphan_present(sb) ||
			  pxt4_has_feature_journal_needs_recovery(sb));

	if (pxt4_has_feature_mmp(sb) && !sb_rdonly(sb)) {
		err = pxt4_multi_mount_protect(sb, le64_to_cpu(es->s_mmp_block));
		if (err)
			goto failed_mount3a;
	}
	...
	if (!test_opt(sb, NOLOAD) && pxt4_has_feature_journal(sb)) {
		err = pxt4_load_and_init_journal(sb, es, ctx);
		if (err)
			goto failed_mount3a;
	...
	if (sbi->s_overhead > pxt4_blocks_count(es))
		sbi->s_overhead = 0;
	/*
	...
	err = pxt4_setup_super(sb, es, sb_rdonly(sb));
	if (err == -EROFS) {
		sb->s_flags |= SB_RDONLY;
	} else if (err)
		goto failed_mount4a;
	 ...
	 */
	if (!(ctx->spec & PXT4_SPEC_mb_optimize_scan)) {
		if (sbi->s_groups_count >= MB_DEFAULT_LINEAR_SCAN_THRESHOLD)
	...
	if (sbi->s_journal)
		sbi->s_journal->j_commit_callback =
			pxt4_journal_commit_callback;

	err = pxt4_percpu_param_init(sbi);
	if (err)
		goto failed_mount6;
	 ...
	 * pxt4_orphan_cleanup. Otherwise the superblock can have an incorrect
...
#ifdef CONFIG_QUOTA
	for (unsigned int i = 0; i < PXT4_MAXQUOTAS; i++)
		kfree(get_qf_name(sb, sbi, i));
#endif
	fscrypt_free_dummy_policy(&sbi->s_dummy_enc_policy);
	brelse(sbi->s_sbh);
...
static int pxt4_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	if (ctx->spec & PXT4_SPEC_s_sb_block)
		sbi->s_sb_block = ctx->s_sb_block;

	ret = __pxt4_fill_super(fc, sb);
	if (ret < 0)
		goto free_sbi;
	...
	if (___ratelimit(&pxt4_mount_msg_ratelimit, "PXT4-fs mount"))
		pxt4_msg(sb, KERN_INFO, "mounted filesystem %pU %s with%s. "
			 "Quota mode: %s.", &sb->s_uuid,
			 sb_rdonly(sb) ? "ro" : "r/w", descr,
			 pxt4_quota_mode(sb));

	/* Update the s_overhead_clusters if necessary */
	pxt4_update_overhead(sb, false);
	return 0;
...
static int pxt4_get_tree(struct fs_context *fc)
{
	return get_tree_bdev(fc, pxt4_fill_super);
}
...
static void pxt4_init_journal_params(struct super_block *sb, journal_t *journal)
{
	 ...
	 */
	journal->j_flags |= JBD3_CYCLE_RECORD;
					     ...
					     unsigned int journal_inum)
{
	...
	pxt4_debug("Journal inode found at %p: %lld bytes\n",
		  journal_inode, journal_inode->i_size);
	return journal_inode;
}

static int pxt4_journal_bmap(journal_t *journal, sector_t *block)
{
	...
	map.m_lblk = *block;
	map.m_len = 1;
	ret = pxt4_map_blocks(NULL, journal->j_inode, &map, 0);
	if (ret <= 0) {
		pxt4_msg(journal->j_inode->i_sb, KERN_CRIT,
			 "journal bmap failed: block %llu ret %d\n",
			 *block, ret);
		jbd3_journal_abort(journal, ret ? ret : -EIO);
		return ret;
	}
	*block = map.m_pblk;
	return 0;
...
static journal_t *pxt4_open_inode_journal(struct super_block *sb,
					  unsigned int journal_inum)
{
	struct inode *journal_inode;
	journal_t *journal;
	...
	journal->j_bmap = pxt4_journal_bmap;
	pxt4_init_journal_params(sb, journal);
	return journal;
}

static struct block_device *pxt4_get_journal_blkdev(struct super_block *sb,
					dev_t j_dev, pxt4_fsblk_t *j_start,
					pxt4_fsblk_t *j_len)
{
	struct buffer_head *bh;
	...
	if ((le16_to_cpu(es->s_magic) != PXT4_SUPER_MAGIC) ||
		...
		pxt4_msg(sb, KERN_ERR, "external journal has bad superblock");
		errno = -EFSCORRUPTED;
		goto out_bh;
	}
	...
	if ((le32_to_cpu(es->s_feature_ro_compat) &
	    ...
	    es->s_checksum != pxt4_superblock_csum(sb, es)) {
		pxt4_msg(sb, KERN_ERR, "external journal has corrupt superblock");
		errno = -EFSCORRUPTED;
		goto out_bh;
	}
...
static journal_t *pxt4_open_dev_journal(struct super_block *sb,
					dev_t j_dev)
{
	journal_t *journal;
	pxt4_fsblk_t j_start;
	pxt4_fsblk_t j_len;
	struct block_device *journal_bdev;
	int errno = 0;

	journal_bdev = pxt4_get_journal_blkdev(sb, j_dev, &j_start, &j_len);
	if (IS_ERR(journal_bdev))
		return ERR_CAST(journal_bdev);

	journal = jbd3_journal_init_dev(journal_bdev, sb->s_bdev, j_start,
	...
	journal->j_private = sb;
	PXT4_SB(sb)->s_journal_bdev = journal_bdev;
	pxt4_init_journal_params(sb, journal);
	return journal;
			     ...
			     unsigned long journal_devnum)
{
	...
	if (journal_inum) {
		journal = pxt4_open_inode_journal(sb, journal_inum);
		if (IS_ERR(journal))
			return PTR_ERR(journal);
	} else {
		journal = pxt4_open_dev_journal(sb, journal_dev);
		if (IS_ERR(journal))
			return PTR_ERR(journal);
	}
	...
	if (!err) {
		...
		if (save && memcmp(((char *) es) + PXT4_S_ERR_START,
				   save, PXT4_S_ERR_LEN)) {
		...
		es->s_state |= cpu_to_le16(PXT4_SB(sb)->s_mount_state &
					   PXT4_ERROR_FS);
...
/* Copy state of PXT4_SB(sb) into buffer for on-disk superblock */
static void pxt4_update_super(struct super_block *sb)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	struct pxt4_super_block *es = sbi->s_es;
	...
	if (percpu_counter_initialized(&sbi->s_freeclusters_counter))
		pxt4_free_blocks_count_set(es,
			PXT4_C2B(sbi, percpu_counter_sum_positive(
	...
	if (sbi->s_add_error_count > 0) {
		...
		if (!es->s_first_error_time && !es->s_first_error_time_hi) {
			__pxt4_update_tstamp(&es->s_first_error_time,
			...
			es->s_first_error_errcode =
				pxt4_errno_to_code(sbi->s_first_error_code);
		}
		__pxt4_update_tstamp(&es->s_last_error_time,
		...
		es->s_last_error_errcode =
				pxt4_errno_to_code(sbi->s_last_error_code);
		/*
				       ...
				       struct pxt4_super_block *es)
{
	...
	err = jbd3_journal_flush(journal, 0);
	if (err < 0)
		goto out;

	if (sb_rdonly(sb) && (pxt4_has_feature_journal_needs_recovery(sb) ||
	    pxt4_has_feature_orphan_present(sb))) {
		if (!pxt4_orphan_file_empty(sb)) {
			pxt4_error(sb, "Orphan file not empty on read-only fs.");
			err = -EFSCORRUPTED;
			goto out;
		}
		pxt4_clear_feature_journal_needs_recovery(sb);
				   ...
				   struct pxt4_super_block *es)
{
	...
	if (j_errno) {
		...
		PXT4_SB(sb)->s_mount_state |= PXT4_ERROR_FS;
		es->s_state |= cpu_to_le16(PXT4_ERROR_FS);
		j_errno = pxt4_commit_super(sb);
		if (j_errno)
			return j_errno;
		pxt4_warning(sb, "Marked fs in need of filesystem check.");
...
{
	return pxt4_journal_force_commit(PXT4_SB(sb)->s_journal);
}
...
static int pxt4_freeze(struct super_block *sb)
{
	...
	if (journal) {
		 ...
		 * flush the journal.
		 */
		error = jbd3_journal_flush(journal, 0);
		if (error < 0)
			goto out;
...
static int __pxt4_remount(struct fs_context *fc, struct super_block *sb)
{
	struct pxt4_fs_context *ctx = fc->fs_private;
	struct pxt4_super_block *es;
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	 ...
	 * two calls to pxt4_should_dioread_nolock() to return inconsistent
	 * values, triggering WARN_ON in pxt4_add_complete_io(). we grab
	 * here s_writepages_rwsem to avoid race between writepages ops and
	 * remount.
	 */
	alloc_ctx = pxt4_writepages_down_write(sb);
	pxt4_apply_options(fc, sb);
	pxt4_writepages_up_write(sb, alloc_ctx);
	...
	if (test_opt2(sb, ABORT))
		pxt4_abort(sb, ESHUTDOWN, "Abort forced by user");
	...
	if ((bool)(fc->sb_flags & SB_RDONLY) != sb_rdonly(sb)) {
		...
		g, le16_to_cpu(pxt4_group_desc_csum(sb, g, gdp)),
			 ...
			 */
			if (es->s_last_orphan || !pxt4_orphan_file_empty(sb)) {
			...
			if (pxt4_has_feature_mmp(sb)) {
				err = pxt4_multi_mount_protect(sb,
...
static int pxt4_reconfigure(struct fs_context *fc)
{
	...
	fc->s_fs_info = PXT4_SB(sb);

	ret = pxt4_check_opt_consistency(fc, sb);
	if (ret < 0)
		return ret;

	ret = __pxt4_remount(fc, sb);
	if (ret < 0)
		return ret;

	pxt4_msg(sb, KERN_INFO, "re-mounted %pU %s. Quota mode: %s.",
...
static int pxt4_mark_dquot_dirty(struct dquot *dquot)
{
	...
	if (pxt4_is_quota_journalled(sb)) {
...
static int pxt4_write_info(struct super_block *sb, int type)
{
	...
	handle = pxt4_journal_start_sb(sb, PXT4_HT_QUOTA, 2);
	if (IS_ERR(handle))
		return PTR_ERR(handle);
...
static inline bool pxt4_check_quota_inum(int type, unsigned long qf_inum)
{
	switch (type) {
	case USRQUOTA:
		return qf_inum == PXT4_USR_QUOTA_INO;
	case GRPQUOTA:
		return qf_inum == PXT4_GRP_QUOTA_INO;
	case PRJQUOTA:
		return qf_inum >= PXT4_GOOD_OLD_FIRST_INO;
	default:
		BUG();
	}
}
			     ...
			     unsigned int flags)
{
	...
	if (!pxt4_check_quota_inum(type, qf_inums[type])) {
		pxt4_error(sb, "Bad quota inum: %lu, type: %d",
				qf_inums[type], type);
		return -EUCLEAN;
	}
	...
	if (IS_ERR(qf_inode)) {
		pxt4_error(sb, "Bad quota inode: %lu, type: %d",
				qf_inums[type], type);
...
int pxt4_enable_quotas(struct super_block *sb)
{
	int type, err = 0;
	unsigned long qf_inums[PXT4_MAXQUOTAS] = {
		le32_to_cpu(PXT4_SB(sb)->s_es->s_usr_quota_inum),
				...
				const char *data, size_t len, loff_t off)
{
	...
	err = pxt4_journal_get_write_access(handle, sb, bh, PXT4_JTR_NONE);
	if (err) {
		brelse(bh);
		return err;
	}
