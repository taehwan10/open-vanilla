		     ...
		     __u64 donor_start, __u64 *len)
{
	...
	if (IS_SWAPFILE(orig_inode) || IS_SWAPFILE(donor_inode)) {
		pxt4_debug("pxt4 move extent: The argument files should not be swap files [ino:orig %lu, donor %lu]\n",
			orig_inode->i_ino, donor_inode->i_ino);
		return -ETXTBSY;
	}

	if (pxt4_is_quota_file(orig_inode) && pxt4_is_quota_file(donor_inode)) {
		pxt4_debug("pxt4 move extent: The argument files should not be quota files [ino:orig %lu, donor %lu]\n",
			orig_inode->i_ino, donor_inode->i_ino);
		return -EOPNOTSUPP;
	}
		  ...
		  __u64 donor_blk, __u64 len, __u64 *moved_len)
{
	...
	while (o_start < o_end) {
		 ...
		 *    pxt4_get_block in move_extent_per_page
		 */
		pxt4_double_up_write_data_sem(orig_inode, donor_inode);
...
out:
	if (*moved_len) {
		pxt4_discard_preallocations(orig_inode, 0);
		pxt4_discard_preallocations(donor_inode, 0);
	}
