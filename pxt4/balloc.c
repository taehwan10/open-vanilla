					   ...
					   struct pxt4_group_desc *gdp)
{
	...
	pxt4_fsblk_t end = start + PXT4_BLOCKS_PER_GROUP(sb) - 1;
	pxt4_fsblk_t itbl_blk_start, itbl_blk_end;
	struct pxt4_sb_info *sbi = PXT4_SB(sb);

	/* This is the number of clusters used by the superblock,
	 * block group descriptors, and reserved block group
	 * descriptor blocks */
	base_clusters = pxt4_num_base_meta_clusters(sb, block_group);
	 ...
	 * accounted below.
	 */
	itbl_blk_start = pxt4_inode_table(sb, gdp);
	...
	if (itbl_blk_start <= end && itbl_blk_end >= start) {
		...
		itbl_cluster_start = PXT4_B2C(sbi, itbl_blk_start - start);
		itbl_cluster_end = PXT4_B2C(sbi, itbl_blk_end - start);
...
static pxt4_fsblk_t pxt4_valid_block_bitmap_padding(struct super_block *sb,
						    pxt4_group_t block_group,
						    struct buffer_head *bh)
{
	...
	next_zero_bit = pxt4_find_next_zero_bit(bh->b_data, bitmap_size, offset);
					    ...
					    pxt4_group_t group)
{
	...
	if (unlikely(group >= PXT4_SB(sb)->s_groups_count))
		return NULL;
					    ...
					    struct buffer_head *bh)
{
	...
	offset = blk - group_first_block;
	if (offset < 0 || PXT4_B2C(sbi, offset) >= max_bit ||
	    PXT4_B2C(sbi, offset + sbi->s_itb_per_group - 1) >= max_bit)
		return blk;
	next_zero_bit = pxt4_find_next_zero_bit(bh->b_data,
			PXT4_B2C(sbi, offset + sbi->s_itb_per_group - 1) + 1,
			PXT4_B2C(sbi, offset));
	if (next_zero_bit <
	    PXT4_B2C(sbi, offset + sbi->s_itb_per_group - 1) + 1)
				      ...
				      struct buffer_head *bh)
{
	...
	if (buffer_verified(bh))
		goto verified;
	if (unlikely(!pxt4_block_bitmap_csum_verify(sb, desc, bh) ||
		     pxt4_simulate_fail(sb, PXT4_SIM_BBITMAP_CRC))) {
	...
	blk = pxt4_valid_block_bitmap_padding(sb, block_group, bh);
	if (unlikely(blk != 0)) {
		pxt4_unlock_group(sb, block_group);
		pxt4_error(sb, "bg %u: block %llu: padding at end of block bitmap is not set",
			   block_group, blk);
		pxt4_mark_group_bitmap_corrupted(sb, block_group,
						 PXT4_GROUP_INFO_BBITMAP_CORRUPT);
		return -EFSCORRUPTED;
			      ...
			      bool ignore_locked)
{
	 ...
	 */
	set_buffer_new(bh);
	trace_pxt4_read_block_bitmap_load(sb, block_group, ignore_locked);
	pxt4_read_bh_nowait(bh, REQ_META | REQ_PRIO |
			    (ignore_locked ? REQ_RAHEAD : 0),
			    pxt4_end_bitmap_read);
	return bh;
verify:
			   ...
			   struct buffer_head *bh)
{
	...
	if (!desc)
		return -EFSCORRUPTED;
	wait_on_buffer(bh);
	pxt4_simulate_fail_bh(sb, bh, PXT4_SIM_BBITMAP_EIO);
	if (!buffer_uptodate(bh)) {
		pxt4_error_err(sb, EIO, "Cannot read block bitmap - "
			       "block_group = %u, block_bitmap = %llu",
...
pxt4_read_block_bitmap(struct super_block *sb, pxt4_group_t block_group)
{
	...
	bh = pxt4_read_block_bitmap_nowait(sb, block_group, false);
	if (IS_ERR(bh))
		return bh;
 ...
 *
 * pxt4_should_retry_alloc() is called when ENOSPC is returned while
...
int pxt4_should_retry_alloc(struct super_block *sb, int *retries)
{
	...
	if (sbi->s_mb_free_pending == 0) {
		...
		return pxt4_has_free_clusters(sbi, 1, 0);
	}
	 ...
	 */
	pxt4_debug("%s: retrying operation after ENOSPC\n", sb->s_id);
 ...
 */
unsigned int pxt4_num_base_meta_blocks(struct super_block *sb,
				       pxt4_group_t block_group)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	unsigned num;
	...
	if (!pxt4_has_feature_meta_bg(sb) ||
		...
		if (num) {
			num += pxt4_bg_num_gdb_nometa(sb, block_group);
			num += le16_to_cpu(sbi->s_es->s_reserved_gdt_blocks);
		}
	} else { /* For META_BG_BLOCK_GROUPS */
		num += pxt4_bg_num_gdb_meta(sb, block_group);
	}
	return num;
}

static unsigned int pxt4_num_base_meta_clusters(struct super_block *sb,
						pxt4_group_t block_group)
{
	return PXT4_NUM_B2C(PXT4_SB(sb), pxt4_num_base_meta_blocks(sb, block_group));
}
