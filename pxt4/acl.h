...
/* acl.c */
struct posix_acl *pxt4_get_acl(struct inode *inode, int type, bool rcu);
int pxt4_set_acl(struct mnt_idmap *idmap, struct dentry *dentry,
...
pxt4_init_acl(handle_t *handle, struct inode *inode, struct inode *dir)
{
	/* usually, the umask is applied by posix_acl_create(), but if
	   pxt4 ACL support is disabled at compile time, we need to do
