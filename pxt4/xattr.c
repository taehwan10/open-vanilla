...
static __le32 pxt4_xattr_hash_entry(char *name, size_t name_len, __le32 *value,
				    size_t value_count);
static __le32 pxt4_xattr_hash_entry_signed(char *name, size_t name_len, __le32 *value,
				    size_t value_count);
...
static const struct xattr_handler * const pxt4_xattr_handler_map[] = {
	...
	[PXT4_XATTR_INDEX_POSIX_ACL_ACCESS]  = &nop_posix_acl_access,
	[PXT4_XATTR_INDEX_POSIX_ACL_DEFAULT] = &nop_posix_acl_default,
#endif
...
#endif
	[PXT4_XATTR_INDEX_HURD]		     = &pxt4_xattr_hurd_handler,
};
...
{
	struct pxt4_inode_info *ei = PXT4_I(ea_inode);
...
static inline const char *pxt4_xattr_prefix(int name_index,
...
check_xattrs(struct inode *inode, struct buffer_head *bh,
	     struct pxt4_xattr_entry *entry, void *end, void *value_start,
	     const char *function, unsigned int line)
{
	...
	if (bh) {
		...
		struct pxt4_xattr_ibody_header *header = value_start;
	...
	while (!IS_LAST_ENTRY(entry)) {
		...
		if (!pxt4_has_feature_ea_inode(inode->i_sb) && ea_ino) {
			err_str = "ea_inode specified without ea_inode feature enabled";
			goto errout;
		}
		if (ea_ino && ((ea_ino == PXT4_ROOT_INO) ||
			       !pxt4_valid_inum(inode->i_sb, ea_ino))) {
...
errout:
	if (bh)
		__pxt4_error_inode(inode, function, line, 0, -err,
				 ...
				 u32 ea_inode_hash, struct inode **ea_inode)
{
	...
	inode = pxt4_iget(parent->i_sb, ea_ino, PXT4_IGET_EA_INODE);
	if (IS_ERR(inode)) {
		err = PTR_ERR(inode);
		pxt4_error(parent->i_sb,
...
void pxt4_evict_ea_inode(struct inode *inode)
{
	...
	while ((oe = mb_cache_entry_delete_or_get(EA_INODE_CACHE(inode),
			pxt4_xattr_inode_get_hash(inode), inode->i_ino))) {
			       ...
			       size_t size)
{
	...
	if (entry) {
		 ...
		 */
		e_hash = pxt4_xattr_hash_entry_signed(entry->e_name, entry->e_name_len,
							&tmp_data, 1);
		/* Still no match - bad */
		if (e_hash != entry->e_hash)
			return -EFSCORRUPTED;
		...
		pr_warn_once("pxt4: filesystem with signed xattr name hash");
	}
	return 0;
}
			...
			char *buffer, size_t buffer_size)
{
	...
	for (; !IS_LAST_ENTRY(entry); entry = PXT4_XATTR_NEXT(entry)) {
		...
		prefix = pxt4_xattr_prefix(entry->e_name_index, dentry);
		if (prefix) {
					  ...
					  struct super_block *sb)
{
	...
	if (pxt4_journal_get_write_access(handle, sb, PXT4_SB(sb)->s_sbh,
					  PXT4_JTR_NONE) == 0) {
		lock_buffer(PXT4_SB(sb)->s_sbh);
		pxt4_set_feature_xattr(sb);
		pxt4_superblock_csum_set(sb);
		unlock_buffer(PXT4_SB(sb)->s_sbh);
		pxt4_handle_dirty_metadata(handle, NULL, PXT4_SB(sb)->s_sbh);
	}
}
...
static int pxt4_xattr_restart_fn(handle_t *handle, struct inode *inode,
			     ...
			     int extra_credits, bool skip_quota)
{
	...
	for (entry = first; !IS_LAST_ENTRY(entry);
		...
		err = pxt4_journal_ensure_credits_fn(handle, credits, credits,
			pxt4_free_metadata_revoke_credits(parent->i_sb, 1),
			pxt4_xattr_restart_fn(handle, parent, bh, block_csum,
					      dirty));
		if (err < 0) {
			 ...
			 int extra_credits)
{
	...
	error = pxt4_journal_get_write_access(handle, inode->i_sb, bh,
					      PXT4_JTR_NONE);
	if (error)
		goto out;
...
retry:
	...
	while (wsize < bufsize) {
		...
		ret = pxt4_journal_get_write_access(handle, ea_inode->i_sb, bh,
						   PXT4_JTR_NONE);
		if (ret)
			goto out;
	...
	ret2 = pxt4_mark_inode_dirty(handle, ea_inode);
					     ...
					     struct inode *inode, u32 hash)
{
	...
	if (!IS_ERR(ea_inode)) {
		...
		if (err) {
			if (pxt4_xattr_inode_dec_ref(handle, ea_inode))
				pxt4_warning_inode(ea_inode,
					  ...
					  struct inode **ret_inode)
{
	...
	if (err) {
		if (pxt4_xattr_inode_dec_ref(handle, ea_inode))
			pxt4_warning_inode(ea_inode, "cleanup dec ref error %d", err);
		iput(ea_inode);
		return err;
	}
				...
				bool is_block)
{
	...
	if (!i->value) {
		 ...
		 */
		if (!is_block && pxt4_has_inline_data(inode)) {
			ret = pxt4_find_inline_data_nolock(inode);
			if (ret) {
...
#define header(x) ((struct pxt4_xattr_header *)(x))
	...
	if (s->base) {
		...
		error = pxt4_journal_get_write_access(handle, sb, bs->bh,
						      PXT4_JTR_NONE);
		if (error)
			goto cleanup;
		lock_buffer(bs->bh);
	...
	if (!IS_LAST_ENTRY(s->first)) {
		...
		if (new_bh) {
			...
			if (new_bh == bs->bh)
				...
				error = pxt4_journal_get_write_access(
						handle, sb, new_bh,
						PXT4_JTR_NONE);
				if (error)
					goto cleanup_dquot;
...
getblk_failed:
			...
			lock_buffer(new_bh);
			error = pxt4_journal_get_create_access(handle, sb,
							new_bh, PXT4_JTR_NONE);
			if (error) {
				unlock_buffer(new_bh);
				error = -EIO;
				goto getblk_failed;
...
int pxt4_xattr_ibody_set(handle_t *handle, struct inode *inode,
				struct pxt4_xattr_info *i,
				struct pxt4_xattr_ibody_find *is)
{
	struct pxt4_xattr_ibody_header *header;
		      ...
		      int flags)
{
	...
	if (pxt4_handle_valid(handle)) {
		...
		if (jbd3_handle_buffer_credits(handle) < credits) {
			error = -ENOSPC;
			goto cleanup;
		}
...
retry_inode:
	...
	pxt4_fc_mark_ineligible(inode->i_sb, PXT4_FC_REASON_XATTR, handle);
...
retry:
	...
	pxt4_fc_mark_ineligible(inode->i_sb, PXT4_FC_REASON_XATTR, NULL);
...
shift:
	...
	if (pxt4_has_inline_data(inode))
		error = pxt4_find_inline_data_nolock(inode);
			    ...
			    int extra_credits)
{
	...
	error = pxt4_journal_ensure_credits(handle, extra_credits,
	...
	if (pxt4_has_feature_ea_inode(inode->i_sb) &&
		...
		error = pxt4_journal_get_write_access(handle, inode->i_sb,
	...
	if (PXT4_I(inode)->i_file_acl) {
		...
		if (IS_ERR(bh)) {
			error = PTR_ERR(bh);
			if (error == -EIO) {
				PXT4_ERROR_INODE_ERR(inode, EIO,
...
/*
 * pxt4_xattr_hash_entry_signed()
 *
 * Compute the hash of an extended attribute incorrectly.
 */
static __le32 pxt4_xattr_hash_entry_signed(char *name, size_t name_len, __le32 *value, size_t value_count)
{
	__u32 hash = 0;
