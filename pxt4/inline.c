...
int pxt4_get_max_inline_size(struct inode *inode)
{
	...
	if (error) {
		pxt4_error_inode_err(inode, __func__, __LINE__, 0, -error,
				 ...
				 struct pxt4_iloc *iloc)
{
	...
	cp_len = min_t(unsigned int, len, PXT4_MIN_INLINE_DATA_SIZE);
				   ...
				   struct inode *inode, unsigned len)
{
	...
	error = pxt4_journal_get_write_access(handle, inode->i_sb, is.iloc.bh,
					      PXT4_JTR_NONE);
	if (error)
		goto out;
...
static int pxt4_read_inline_folio(struct inode *inode, struct folio *folio)
{
	void *kaddr;
	int ret = 0;
	size_t len;
...
int pxt4_readpage_inline(struct inode *inode, struct folio *folio)
{
	 ...
	 */
	if (!folio->index)
		ret = pxt4_read_inline_folio(inode, folio);
...
retry:
	...
	if (pxt4_should_dioread_nolock(inode)) {
		...
		ret = __block_write_begin(&folio->page, from, to, pxt4_get_block);
				  ...
				  struct page **pagep)
{
	...
	ret = pxt4_journal_get_write_access(handle, inode->i_sb, iloc.bh,
					    PXT4_JTR_NONE);
	if (ret)
		goto out;
...
out:
	...
	return ret;
convert:
	return pxt4_convert_inline_data_to_extent(mapping, inode);
}
			       ...
			       unsigned copied, struct folio *folio)
{
	...
	if (likely(copied)) {
		 ...
		 * pxt4_write_begin() called
		 * pxt4_try_to_write_inline_data()
		 */
		 ...
		 * beyond i_size.
		 */
		pxt4_update_inode_size(inode, pos + copied);
	}
	folio_unlock(folio);
	folio_put(folio);
...
out:
	 ...
	 */
	if (pos + len > inode->i_size && pxt4_can_truncate(inode))
		pxt4_orphan_add(handle, inode);
				     ...
				     void *inline_start, int inline_size)
{
	...
	err = pxt4_journal_get_write_access(handle, dir->i_sb, iloc->bh,
					    PXT4_JTR_NONE);
	if (err)
		return err;
	pxt4_insert_dentry(dir, inode, de, inline_size, fname);
				  ...
				  struct pxt4_iloc *iloc)
{
	...
	if (new_size - old_size <= pxt4_dir_rec_len(1, NULL))
		return -ENOSPC;
					   ...
					   struct pxt4_iloc *iloc)
{
	...
	lock_buffer(data_bh);
	error = pxt4_journal_get_create_access(handle, inode->i_sb, data_bh,
					       PXT4_JTR_NONE);
	if (error) {
...
out:
	...
	ret2 = pxt4_mark_inode_dirty(handle, dir);
	...
	while (pos < inline_size) {
		...
		if (pos == 0) {
			...
			fake.rec_len = pxt4_rec_len_to_disk(
					  pxt4_dir_rec_len(fake.name_len, NULL),
					  inline_size);
			 ...
			 int *has_inline_data)
{
	 ...
	 */
	dotdot_offset = pxt4_dir_rec_len(1, NULL);
	dotdot_size = dotdot_offset + pxt4_dir_rec_len(2, NULL);
...
void *pxt4_read_inline_link(struct inode *inode)
{
	struct pxt4_iloc iloc;
	int ret, inline_size;
	void *link;
	...
	ret = pxt4_read_inline_data(inode, link, inline_size, &iloc);
	if (ret < 0) {
		kfree(link);
		goto out;
	}
			     ...
			     int *has_inline_data)
{
	...
	err = pxt4_generic_delete_entry(dir, de_del, bh,
